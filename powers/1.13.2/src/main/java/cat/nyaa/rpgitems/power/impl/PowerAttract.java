...
public class PowerAttract extends BasePower implements PowerTick, PowerLeftClick, PowerRightClick, PowerPlain {
    ...
    private PowerResult<Void> attract(Player player, ItemStack stack) {
        ...
        if (!triggers.contains(TICK) && !stack.equals(player.getInventory().getItemInMainHand())) {
            return PowerResult.noop();
        }
        ...
        if (!getItem().consumeDurability(stack, attractingTickCost)) return null;
        for (Entity e : entities) {
            if (e instanceof LivingEntity
                        && (attractPlayer || !(e instanceof Player))) {
                if (!getItem().consumeDurability(stack, attractingEntityTickCost)) break;

 ...
 * 2. auto triggered when {@link HIT_TAKEN} is enabled with a chance of {@link #chance} and a cooldown of {@link #cooldownpassive}
 * </p>
 */
...
public class PowerDeflect extends BasePower implements PowerHitTaken, PowerRightClick, PowerLeftClick, PowerPlain {
    ...
    public void init(ConfigurationSection section) {
        ...
        if (passive) {
            triggers.add(HIT_TAKEN);
        }
        if (initiative) {
            triggers.add(isRight ? RIGHT_CLICK : LEFT_CLICK);
        }
        super.init(section);
    }
    ...
    public PowerResult<Double> takeHit(Player target, ItemStack stack, double damage, EntityDamageEvent event) {
        ...
        if (!activated) {
            if (!triggers.contains(HIT_TAKEN)

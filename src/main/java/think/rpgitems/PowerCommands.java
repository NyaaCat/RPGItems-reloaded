...
public class PowerCommands extends RPGCommandReceiver {
    ...
    private static Pair<NamespacedKey, Class<? extends Power>> getPowerClass(
            CommandSender sender, String powerStr) {
        try {
    ...
    public List<String> addCompleter(CommandSender sender, Arguments arguments) {
        ...
        switch (arguments.remains()) {
            ...
            case 2:
                completeStr.addAll(
                        PowerManager.getPowers().keySet().stream()
            ...
            default:
                ...
                Pair<NamespacedKey, Class<? extends Power>> powerClass =
                        getPowerClass(sender, powerKey);
                if (powerClass != null) {
    ...
    public void add(CommandSender sender, Arguments args) {
        ...
        try {
            ...
            plugin.getLogger()
                    .log(
                            Level.WARNING,
                            "Error adding power " + powerStr + " to item " + itemStr + " " + item,
                            e);
    ...
    public List<String> propCompleter(CommandSender sender, Arguments arguments) {
        ...
        switch (arguments.remains()) {
            ...
            case 2:
                RPGItem item = getItem(arguments.nextString(), sender);
                completeStr.addAll(
                        IntStream.range(0, item.getPowers().size())
                                .mapToObj(i -> i + "-" + item.getPowers().get(i).getNamespacedKey())
                                .collect(Collectors.toList()));
                break;
            default:
    ...
    public static void showPower(CommandSender sender, int nth, RPGItem item, Power power) {
        msgs(
                ...
                "message.power.show",
                nth,
                power.getLocalizedName(sender),
                power.getNamespacedKey().toString(),
                power.displayText() == null
                        ? I18n.getInstance(sender).format("message.power.no_display")
                        : power.displayText(),
                power.getTriggers().stream().map(Trigger::name).collect(Collectors.joining(",")));
        NamespacedKey powerKey = item.getPropertyHolderKey(power);
        PowerManager.getProperties(powerKey)
                .forEach((name, prop) -> showProp(sender, powerKey, prop.getValue(), power));
    }
    ...
    public List<String> reorderCompleter(CommandSender sender, Arguments arguments) {
        ...
        switch (arguments.remains()) {
            ...
            case 3:
                ...
                completeStr.addAll(
                        IntStream.range(0, item1.getPowers().size())
                                .filter(i -> i != i1)
                                .mapToObj(
                                        i -> i + "-" + item1.getPowers().get(i).getNamespacedKey())
                                .collect(Collectors.toList()));
                break;
        }
        return filtered(arguments, completeStr);
    ...
    public void list(CommandSender sender, Arguments args) {
        ...
        stream.forEach(
                power -> {
                    ...
                    msgs(
                            sender,
                            "message.power.description",
                            PowerManager.getDescription(power, null));
                    PowerManager.getProperties(power)
                            .forEach((name, mp) -> showProp(sender, power, mp.getValue(), null));

...
public class RPGItem {
    ...
    private void restore(ConfigurationSection s)
            ...
            for (String sectionKey : conditionList.getKeys(false)) {
                ...
                String conditionName =
                        Objects.requireNonNull(
                                Objects.requireNonNull(section).getString("conditionName"));
        ...
        if (markerList != null) {
            for (String sectionKey : markerList.getKeys(false)) {
                ...
                String markerName =
                        Objects.requireNonNull(
                                Objects.requireNonNull(section).getString("markerName"));
        ...
        if (s.isBoolean("forceBar")) {
            setHasDurabilityBar(
                    getItem().getMaxDurability() == 0
                            || s.getBoolean("forceBar")
                            || isCustomItemModel());
        }
    ...
    private void loadTrigger(ConfigurationSection section, String triggerName)
        ...
        if (base == null) {
            plugin.getLogger()
                    .warning("Unknown base trigger:" + baseTrigger + " on item " + this.name);
    ...
    public void updateItem(ItemStack item, boolean loreOnly) {
        ...
        if (item.hasItemMeta()
                ...
                && Objects.requireNonNull(item.getItemMeta().getLore())
                        .contains("mcMMO Ability Tool")) lore.add("mcMMO Ability Tool");
        ...
        if (enchantMap != null) {
            for (Entry<Enchantment, Integer> e : enchantMap.entrySet()) {
                meta.addEnchant(
                        e.getKey(), Math.max(meta.getEnchantLevel(e.getKey()), e.getValue()), true);
            }
        }
                    ...
                    case NUMERIC_MINUS_ONE:
                        {
                            ...
                            out.append(
                                    ratio < 0.1
                                            ? ChatColor.RED
                                            : ratio < 0.3 ? ChatColor.YELLOW : ChatColor.GREEN);
                    ...
                    case DEFAULT:
                        {
                            ...
                            for (int i = 0; i < boxCount; i++) {
                                out.append(
                                        i < mid
                                                ? ChatColor.GREEN
                                                : i == mid ? ChatColor.YELLOW : ChatColor.RED);
                                out.append(boxChar);
                            }
                            break;
                        }
        ...
        switch (barFormat) {
            ...
            case NUMERIC_BIN:
                ...
                return String.format(
                                String.format("0b%%%ds", binLen),
                                Integer.toBinaryString(durability))
                        .replace(' ', '0');
            case NUMERIC_BIN_MINUS_ONE:
                int binLenM1 = Integer.toBinaryString(maxDurability - 1).length();
                return String.format(
                                String.format("0b%%%ds", binLenM1),
                                Integer.toBinaryString(durability - 1))
                        .replace(' ', '0');
        }
                        ...
                        getDamageMin() != getDamageMax()
                                ? (getDamageMin()
                                        + ThreadLocalRandom.current()
                                                .nextInt(getDamageMax() - getDamageMin() + 1))
                                : getDamageMin();
    ...
    private <
        ...
        RPGItemsPowersPostFireEvent<TEvent, TPower, TPimpl, TResult, TReturn> postFire =
                new RPGItemsPowersPostFireEvent<>(
                        player, itemStack, event, this, trigger, resultMap, ret);
    ...
    public List<String> getTooltipLines() {
        ...
        if (isShowArmourLore()) {
            ...
            if ((getDamageMin() != 0 || getDamageMax() != 0)
                    && getDamageMode() != DamageMode.VANILLA) {
            ...
            if (damageStr != null) {
                armorMinLen =
                        Math.max(
                                armorMinLen, Utils.getStringWidth(ChatColor.stripColor(damageStr)));
            }
        }
    ...
    public Event.Result checkPermission(Player p, boolean showWarn) {
        if (isHasPermission() && !p.hasPermission(getPermission())) {
            if (showWarn)
                p.sendMessage(
                        I18n.getInstance(p.getLocale())
                                .format("message.error.permission", getDisplayName()));
    ...
    public void print(CommandSender sender, boolean advance) {
        ...
        if (sender instanceof Player) {
            ...
            new Message("")
                    .append(
                            I18n.getInstance(((Player) sender).getLocale())
                                    .format("message.item.print"),
                            toItemStack())
                    .send(sender);
        } else {
            List<String> lines = getTooltipLines();
        ...
        if (isCustomItemModel()) {
            sender.sendMessage(
                    I18n.formatDefault(
                            "message.print.customitemmodel",
                            getItem().name() + ":" + getDataValue()));
        }
    ...
    public BaseComponent getComponent(String locale) {
        ...
        msg.setClickEvent(
                new ClickEvent(ClickEvent.Action.SUGGEST_COMMAND, "/rpgitem " + getName()));
        HoverEvent hover =
                new HoverEvent(
                        HoverEvent.Action.SHOW_ITEM,
                        new BaseComponent[] {
                            new TextComponent(ItemStackUtils.itemToJson(toItemStack()))
                        });
            ...
            if (getters.containsKey(method)) {
                ...
                if (type == int.class
                    ...
                    for (Modifier<Double> numberModifier : numberModifiers) {
                        RgiParameter param =
                                new RgiParameter<>(orig.getItem(), orig, stack, origValue);

...
public class EquipmentCondition extends BaseCondition<Void> {
    ...
    @Property public ItemStack itemStack;

    @Property public String rpgitem;

    @Property public boolean matchAllSlot = false;

    @Property public boolean requireEmpty = false;
    ...
    public PowerResult<Void> check(
        ...
        if (slots.isEmpty()) {
            ...
            if (matchAllSlot) {
                return itemStacks.stream().allMatch(this::match)
                        ? PowerResult.ok()
                        : PowerResult.fail();
            } else {
                return itemStacks.stream().anyMatch(this::match)
                        ? PowerResult.ok()
                        : PowerResult.fail();
            }
        } else {

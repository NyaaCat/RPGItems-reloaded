...
public class PowerManager {
    ...
    private static void registerPower(Class<? extends Power> clazz) {
        ...
        try {
            ...
            if (!Optional.ofNullable(metas.get(clazz)).map(c -> c.note().isEmpty()).orElse(false)) {
                RPGItems.plugin
                        .getLogger()
                        .log(Level.WARNING, "Note: {0}", metas.get(clazz).note());
            } else {
                        ...
                        Collectors.toMap(
                                ...
                                p -> {
                                    ...
                                    Pair<Method, PropertyInstance> pair =
                                            Pair.of(
                                                    !Power.class.isAssignableFrom(cls)
                                                                    || metas.get(cls).marker()
                                                            ? null
                                                            : methods.stream()
                                                                    .filter(
                                                                            m ->
                                                    ...
                                                    PropertyInstance.from(
                                                            p.getKey(),
                                                            p.getValue(),
                                                            p.getValue().order() < requiredOrder));
    ...
    public static <T extends PropertyHolder> void registerPackage(
            ...
            String basePackage,
            Class<T> tClass,
            Consumer<Class<? extends T>> register) {
    ...
    public static void addDescriptionResolver(
            Plugin plugin,
            String locale,
            BiFunction<NamespacedKey, String, String> descriptionResolver) {
    ...
    public static void setPowerProperty(
        ...
        try {
            ...
            throw new AdminCommands.CommandException(
                    "internal.error.invalid_command_arg", e); // TODO
        }

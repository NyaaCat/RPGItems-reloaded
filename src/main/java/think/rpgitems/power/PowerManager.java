...
public class PowerManager {
  private static final Map<
          Class<? extends PropertyHolder>, Map<String, Pair<Method, PropertyInstance>>>
  ...
  private static <T extends PropertyHolder> void register(
      Class<? extends T> clazz, BiMap<NamespacedKey, Class<? extends T>> registry) {
    NamespacedKey key = null;
    try {
      registerMetas(clazz);
  ...
  private static Map<String, Pair<Method, PropertyInstance>> scanProperties(
      Class<? extends PropertyHolder> cls) {
    ...
    List<Pair<Field, Property>> collect =
        getAllFields(cls).stream()
        ...
        .collect(
            Collectors.toMap(
                ...
                p -> {
                  ...
                  Pair<Method, PropertyInstance> pair =
                      Pair.of(
                          !Power.class.isAssignableFrom(cls) || metas.get(cls).marker()
                              ? null
                              : methods.stream()
                                  .filter(
                                      m ->
                                          m.getParameterCount() == 0
                                              && (m.getName()
                                                      ...
                                                      .equals("get" + name.toLowerCase(Locale.ROOT))
                                                  || m.getName()
                                                      .toLowerCase(Locale.ROOT)
                                                      .equals("is" + name.toLowerCase(Locale.ROOT))
                                                  || m.getName()
                                                      .toLowerCase(Locale.ROOT)
                                  ...
                                  .reduce(
                                      (a, b) -> {
                                        throw new IllegalArgumentException(
                                            "Duplicated property gettor found:"
                                                + p.getKey()
                                                + " "
                                                + name
                                                + " "
                                  ...
                                  .orElseThrow(
                                      () ->
                                          new IllegalArgumentException(
                                              "No property getter found: "
                                                  + p.getKey()
                                                  + " "
                                                  + name)),
                          PropertyInstance.from(
                              p.getKey(), p.getValue(), p.getValue().order() < requiredOrder));
                  // RPGItems.plugin.cfg.enabledLanguages.forEach(lang ->
                  // PowerManager.getDescription())
                  return pair;
                }));
  }

  public static <T extends PropertyHolder> void registerPackage(
      Plugin plugin, String basePackage, Class<T> tClass, Consumer<Class<? extends T>> register) {
    Class<? extends T>[] classes = ClassPathUtils.scanSubclasses(plugin, basePackage, tClass);
    List<Class<? extends T>> classList =
        Arrays.stream(classes)
            .filter(c -> c.getAnnotation(Meta.class) != null)
            .collect(Collectors.toList());
    registerList(plugin, classList, register);
  }

  public static <T extends PropertyHolder> void registerList(
      Plugin plugin, List<Class<? extends T>> list, Consumer<Class<? extends T>> register) {
  ...
  public static void registerPowers(Plugin plugin, String basePackage) {
    registerPackage(
        ...
        basePackage,
        Power.class,
        (Consumer<Class<? extends Power>>) PowerManager::registerPower);
  }
  ...
  public static void registerConditions(Plugin plugin, String basePackage) {
    registerPackage(
        ...
        basePackage,
        Condition.class,
        (Consumer<Class<? extends Condition>>) PowerManager::registerCondition);
  }
  ...
  public static void registerMarkers(Plugin plugin, String basePackage) {
    registerPackage(
        ...
        basePackage,
        Marker.class,
        (Consumer<Class<? extends Marker>>) PowerManager::registerMarker);
  }
  ...
  public static void registerModifiers(Plugin plugin, String basePackage) {
    registerPackage(
        ...
        basePackage,
        Modifier.class,
        (Consumer<Class<? extends Modifier>>) PowerManager::registerModifier);
  }

  public static void addDescriptionResolver(
      Plugin plugin, BiFunction<NamespacedKey, String, String> descriptionResolver) {
    addDescriptionResolver(plugin, RPGItems.plugin.cfg.language, descriptionResolver);
  }

  public static void addDescriptionResolver(
      Plugin plugin, String locale, BiFunction<NamespacedKey, String, String> descriptionResolver) {
  ...
  public static NamespacedKey parseKey(String powerStr) throws UnknownExtensionException {
    if (!powerStr.contains(":"))
      return keyCache.computeIfAbsent(
          powerStr.toLowerCase(), s -> new NamespacedKey(RPGItems.plugin, s));
  ...
  public static void setPowerProperty(
      CommandSender sender, PropertyHolder power, String field, String value)
  ...
  public static List<String> getAcceptedValue(
      Class<? extends PropertyHolder> cls, AcceptedValue anno) {
  ...
  @SuppressWarnings("unused")
  public static Map<Class<? extends PropertyHolder>, Map<String, Pair<Method, PropertyInstance>>>
      getProperties() {
  ...
  public static Map<String, Pair<Method, PropertyInstance>> getProperties(
      Class<? extends PropertyHolder> cls) {
  ...
  public static Class<? extends Condition<?>> getCondition(NamespacedKey key) {
    return (Class<? extends Condition<?>>)
        conditions.get(overrides.computeIfAbsent(key, Function.identity()));
  }
  ...
  public static String getDescription(String locale, NamespacedKey power, String property) {
    ...
    return PowerManager.descriptionResolvers
        .get(plugin, locale.toLowerCase(Locale.ROOT))

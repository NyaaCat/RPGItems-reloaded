...
public abstract class RPGCommandReceiver extends CommandReceiver {
    ...
    public static List<String> resolvePropertyValueSuggestion(
            RPGItem item,
            Class<? extends Power> power,
            String propertyName,
            String last,
            boolean hasNamePrefix) {
        try {
            return resolvePropertyValueSuggestion(
                    item, power, power.getField(propertyName), last, hasNamePrefix);
    ...
    public static List<String> resolvePropertyValueSuggestion(
        ...
        if (bc != null) {
            return Stream.of(bc.trueChoice(), bc.falseChoice())
                    .map(s -> (hasNamePrefix ? propertyField.getName() + ":" : "") + s)
        ...
        if (Collection.class.isAssignableFrom(propertyField.getType())) {
            ...
            if (listArg.equals(Trigger.class)) {
                return resolveEnumListValue(
                        power,
                        propertyField,
                        new ArrayList<>(Trigger.keySet()),
                        last,
                        hasNamePrefix);
            }
            if (!listArg.isEnum()) {
                if (propertyField.getName().equalsIgnoreCase("conditions")) {
                    ...
                    return resolveEnumListValue(
                            power, propertyField, conditionIds, last, hasNamePrefix);
                }
                if (propertyField.getName().equalsIgnoreCase("selectors")) {
                    ...
                    List<String> selectorIds =
                            selectors.stream().map(Selector::id).collect(Collectors.toList());
                    return resolveEnumListValue(
                            power, propertyField, selectorIds, last, hasNamePrefix);
                }
                return Collections.emptyList();
            }
            List<String> enumValues =
                    Stream.of(((Class<? extends Enum>) listArg).getEnumConstants())
                            .map(Enum::name)
                            .collect(Collectors.toList());
        ...
        if (as != null) {
            return PowerManager.getAcceptedValue(power, as).stream()
                    .map(s -> (hasNamePrefix ? propertyField.getName() + ":" : "") + s)
                    .filter(s -> s.startsWith(last))
                    .collect(Collectors.toList());
        }
        ...
        if (propertyField.getType().isEnum()) {
            return Stream.of(propertyField.getType().getEnumConstants())
                    .map(s -> (hasNamePrefix ? propertyField.getName() + ":" : "") + s.toString())
                    .filter(s -> s.startsWith(last))
                    .collect(Collectors.toList());
        }
    ...
    public static List<String> resolveEnumListValue(
            Class<? extends Power> power,
            Field propertyField,
            List<String> enumValues,
            String last,
        ...
        List<String> currentVaules =
                Stream.of(currentValuesStr.split(",")).collect(Collectors.toList());
        ...
        if (Set.class.isAssignableFrom(propertyField.getType())
                || (as != null && as.preset() == Preset.TRIGGERS)) {
            enumValues.removeAll(currentVaules);
        }
        return resolveEnumCompletion(
                enumValues, last, hasNamePrefix, currentVaules, incompleteValue);
    }

    public static List<String> resolveEnumCompletion(
        ...
        boolean next =
                (currentVaules.isEmpty() && !hasNamePrefix)
                        || base.endsWith(":")
                        || base.endsWith(",");
        return enumValues.stream()
                .filter(n -> n.startsWith(incompleteValue))
                .map(n -> base + (next ? "" : ",") + n)
                .collect(Collectors.toList());
    }

    public static List<String> resolveProperties(
            CommandSender sender,
            RPGItem item,
            Class<? extends PropertyHolder> power,
        ...
        for (Map.Entry<String, Pair<Method, PropertyInstance>> prop : argMap.entrySet()) {
            ...
            if (value != null || isTrivialProperty(meta, name)) {
        ...
        return resolvePropertiesSuggestions(
                sender, item, last, powerKey, argMap, settled, required);
    }
    ...
    public static List<String> resolvePropertiesSuggestions(
            CommandSender sender,
            RPGItem item,
            String last,
            NamespacedKey powerKey,
            Map<String, Pair<Method, PropertyInstance>> argMap,
            Set<Field> settled,
            List<Field> required) {
        if (argMap.keySet().stream()
                .anyMatch(f -> last.startsWith(f + ":"))) { // we are suggesting a value as we have
            // the complete property name
            String currentPropertyName = last.split(":")[0];
            actionBarTip(sender, powerKey, currentPropertyName);
            return resolvePropertyValueSuggestion(
                    item, PowerManager.getPower(powerKey), currentPropertyName, last, true);
        }
        ...
        suggestions =
                argMap.values().stream()
                        .filter(s -> !settled.contains(s.getValue().field()))
                            ...
                            () -> {
                                String description =
                                        PowerManager.getDescription(
                                                ((Player) sender).getLocale(), power, property);
                                if (description == null) {
                                    return;
                                }
                                new Message(description)
                                        .send((Player) sender, Message.MessageType.ACTION_BAR);
                            });
        }
    }
    ...
    public static void showProp(
        ...
        msgs(
                sender,
                "message.propertyHolder.property",
                name,
                Strings.isNullOrEmpty(desc)
                        ? I18n.getInstance(locale).format("message.propertyHolder.no_description")
                        : desc);
        if (powerObj != null) {
            msgs(
                    sender,
                    "message.propertyHolder.property_value",
                    Utils.getProperty(powerObj, name, prop.field()));
        }
    }

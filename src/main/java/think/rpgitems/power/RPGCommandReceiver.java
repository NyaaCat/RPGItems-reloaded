...
public abstract class RPGCommandReceiver extends CommandReceiver {
    ...
    if (Collection.class.isAssignableFrom(propertyField.getType())) {
      ...
      if (listArg.equals(Trigger.class)) {
        return resolveEnumListValue(
            power, propertyField, new ArrayList<>(Trigger.keySet()), last, hasNamePrefix);
      }
      if (!listArg.isEnum()) {
  ...
  public static List<String> resolveEnumCompletion(
    ...
    boolean next =
        (currentVaules.isEmpty() && !hasNamePrefix) || base.endsWith(":") || base.endsWith(",");
  ...
  public static List<String> resolvePropertiesSuggestions(
    ...
    if (argMap.keySet().stream()
        .anyMatch(
            f ->
                last.startsWith(
                    f + ":"))) { // we are suggesting a value as we have the complete property name
    ...
    if (sender instanceof Player) {
      Bukkit.getScheduler()
          .runTask(
              ...
              () -> {
                String description =
                    PowerManager.getDescription(((Player) sender).getLocale(), power, property);
                if (description == null) {
                  return;
                }

...
public class BeamHit<TEvent extends Event, TResult, TReturn>
    extends Trigger<TEvent, PowerBeamHit, TResult, TReturn> {
  ...
  public PowerResult<TResult> run(
      PowerBeamHit powerBeamHit, Player player, ItemStack i, TEvent event) {
    if (event instanceof BeamHitBlockEvent) {
      ...
      return (PowerResult<TResult>)
          powerBeamHit.hitEntity(player, i, event1.getEntity(), event1.getDamage(), event1);

...
public class BeamHit<TEvent extends Event, TResult, TReturn>
  ...
  public PowerResult<TResult> run(
      Power power, PowerBeamHit<Power> powerBeamHit, Player player, ItemStack i, TEvent event) {
    if (event instanceof BeamHitBlockEvent) {
      ...
      return (PowerResult<TResult>)
          powerBeamHit.hitEntity(power, player, i, event1.getEntity(), event1.getDamage(), event1);

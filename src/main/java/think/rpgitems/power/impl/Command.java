 ...
 * Power command.
 *
 * <p>The item will run {@link #command} on click giving the permission {@link #permission} just for
 * the use of the command.
 */
@Meta(
        defaultTrigger = "RIGHT_CLICK",
        generalInterface = PowerPlain.class,
        implClass = Command.Impl.class)
public class Command extends BasePower {
    ...
    public static String handlePlayerPlaceHolder(Player player, String cmd) {
        ...
        cmd =
                cmd.replaceAll(
                        "\\{player\\.yaw}", Float.toString(90 + player.getEyeLocation().getYaw()));
        cmd =
                cmd.replaceAll(
                        "\\{player\\.pitch}", Float.toString(-player.getEyeLocation().getPitch()));
        ...
        if (section.isBoolean("isRight")) {
            ...
            triggers =
                    Collections.singleton(
                            isRight ? BaseTriggers.RIGHT_CLICK : BaseTriggers.LEFT_CLICK);
        }
        super.init(section);
    }
    ...
    public static class Impl extends Base
            implements PowerRightClick<Command>,
                    PowerLeftClick<Command>,
                    PowerSprint<Command>,
                    PowerSneak<Command>,
                    PowerHurt<Command>,
                    PowerHitTaken<Command>,
                    PowerPlain<Command>,
                    PowerBowShoot<Command> {
        @Override
        public PowerResult<Void> leftClick(
                Command power, Player player, ItemStack stack, PlayerInteractEvent event) {
        ...
        public PowerResult<Void> sneak(
                Command power, Player player, ItemStack stack, PlayerToggleSneakEvent event) {
            return fire(power, player, stack);
        }

        @Override
        public PowerResult<Void> sprint(
                Command power, Player player, ItemStack stack, PlayerToggleSprintEvent event) {
            return fire(power, player, stack);
        }

        @Override
        public PowerResult<Void> hurt(
                Command power, Player target, ItemStack stack, EntityDamageEvent event) {
        ...
        public PowerResult<Float> bowShoot(
                Command power, Player player, ItemStack stack, EntityShootBowEvent event) {

...
public class Beam extends BasePower {
  ...
  private static class MovingTask extends BukkitRunnable {
      ...
      public void run() {
        try {
          ...
          while ((lengthToSpawn -= lengthPerSpawn) > 0) {
            hitMob.addAll(
                tryHit(fromEntity, lastLocation, itemStack, bounced && hitSelfWhenBounced, hitMob));
            ...
            if (gravity != 0 && (homing == 0 || currentTick.get() < ticksBeforeHoming)) {
                  ...
                  if (!MovingTask.this.effectOnly) {
                    BeamHitBlockEvent beamHitBlockEvent =
                        new BeamHitBlockEvent(
                            player, fromEntity, block, lastLocation, itemStack, triggerDepth);
    ...
    private void makeBounce(Block block, Vector towards, Vector step, Location lastLocation) {
      RayTraceResult rayTraceResult =
          block.rayTrace(lastLocation, step, towards.length() * 2, FluidCollisionMode.NEVER);
      if (rayTraceResult == null) {
        ...
        if (!transp.contains(relative.getType())) {
          RayTraceResult relativeResult =
              relative.rayTrace(lastLocation, step, towards.length() * 2, FluidCollisionMode.NEVER);
          if (relativeResult != null) {
            ...
            towards
                .rotateAroundAxis(
                    relativeResult.getHitBlockFace().getDirection(), Math.toRadians(180))
                .multiply(-1);
    ...
    private void spawnParticle(Entity from, World world, Location lastLocation, int i) {
      ...
      if (this.mode.equals(Mode.PROJECTILE)
        ...
        extraData =
            new Particle.DustOptions(getNextColor(), ((Particle.DustOptions) extraData).getSize());
      }
        ...
        if (entity instanceof LivingEntity) {
          BeamHitEntityEvent beamHitEntityEvent =
              new BeamHitEntityEvent(
                  ...
                  loc,
                  getBoundingBox(loc),
                  towards.clone().normalize().multiply(getNextLength(spawnedLength, length * 20)),
                  triggerDepth);
          Bukkit.getPluginManager().callEvent(beamHitEntityEvent);
          double damage = beamHitEntityEvent.getDamage();
          if (damage > 0) {
            LightContext.putTemp(
                from.getUniqueId(), DAMAGE_SOURCE, power.getNamespacedKey().toString());
                ...
                .filter(
                    entity ->
                        entity instanceof LivingEntity
                            ...
                            && !entity.isDead()
                            && from.getLocation().distance(entity.getLocation()) < range)
        ...
        .filter(
            livingEntity -> {
              switch (homingTarget) {
                ...
                case PLAYERS:
                  return livingEntity instanceof Player
                      && !((Player) livingEntity).getGameMode().equals(GameMode.SPECTATOR);
                case ALL:
                  return !(livingEntity instanceof Player)
                      || !((Player) livingEntity).getGameMode().equals(GameMode.SPECTATOR);
              }
  ...
  public static class Impl
    ...
    public PowerResult<Double> takeHit(
        Beam power, Player target, ItemStack stack, double damage, EntityDamageEvent event) {
    ...
    public PowerResult<Void> hitBlock(
        Beam power, Player player, ItemStack stack, Location location, BeamHitBlockEvent event) {
      if (event.getDepth() >= 1) return PowerResult.noop();
    ...
    @Override
    public PowerResult<Void> fire(
        Beam power, Player player, ItemStack stack, LivingEntity entity, @Nullable Double value) {

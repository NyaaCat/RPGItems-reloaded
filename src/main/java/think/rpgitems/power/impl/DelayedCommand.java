 ...
 *
 * <p>The item will run {@link #getCommand()} on click with a {@link #delay} giving the permission
 * {@link #getPermission()} just for the use of the command.
 */
...
public class DelayedCommand extends Command {
  ...
  @Property public boolean cmdInPlace = false;
  ...
  public static class Impl extends Command.Base
      implements PowerRightClick<DelayedCommand>,
          PowerLeftClick<DelayedCommand>,
          PowerSprint<DelayedCommand>,
          PowerSneak<DelayedCommand>,
          PowerHurt<DelayedCommand>,
          PowerPlain<DelayedCommand> {
    @Override
    public PowerResult<Void> leftClick(
        DelayedCommand power, final Player player, ItemStack stack, PlayerInteractEvent event) {
    ...
    public PowerResult<Void> sneak(
        DelayedCommand power, Player player, ItemStack stack, PlayerToggleSneakEvent event) {
      return fire(power, player, stack);
    }

    @Override
    public PowerResult<Void> sprint(
        DelayedCommand power, Player player, ItemStack stack, PlayerToggleSprintEvent event) {
      return fire(power, player, stack);
    }

    @Override
    public PowerResult<Void> hurt(
        DelayedCommand power, Player target, ItemStack stack, EntityDamageEvent event) {

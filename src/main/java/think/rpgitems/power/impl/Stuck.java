...
public class Stuck extends BasePower {
                            ...
                            try {
                                if (stucked.get(e.getPlayer().getUniqueId(), () -> Long.MIN_VALUE)
                                    ...
                                    if (e.getCause() != PlayerTeleportEvent.TeleportCause.COMMAND) {
                                        e.getPlayer()
                                                .sendMessage(I18n.formatDefault("message.stuck"));
                                        e.setCancelled(true);
                                    }
                                }
                            } catch (ExecutionException ex) {
    ...
    public static class Impl
        ...
        public PowerResult<Void> fire(Stuck power, Player player, ItemStack stack) {
            ...
            entities.forEach(
                    entity -> {
                        if (!power.getItem().consumeDurability(stack, power.getCostPerEntity()))
                            return;
                        stucked.put(entity.getUniqueId(), System.currentTimeMillis());
                        entity.addPotionEffect(
                                new PotionEffect(PotionEffectType.SLOW, power.getDuration(), 10),
                                true);
                        //                    entity.addPotionEffect(new
                        // PotionEffect(PotionEffectType.JUMP,
                        // duration, 128), true);
                    });

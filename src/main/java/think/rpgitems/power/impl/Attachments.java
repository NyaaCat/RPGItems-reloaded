...
@Meta(
    ...
    implClass = Attachments.Impl.class)
public class Attachments extends BasePower {

  @Property public List<EquipmentSlot> allowedSlots;

  @Property public List<Integer> allowedInvSlots;

  @Property public int limit;

  @Property public Set<String> allowedItems;
  ...
  public static class Impl
      implements PowerTick<Attachments>,
          PowerRightClick<Attachments>,
          PowerLeftClick<Attachments>,
          PowerOffhandClick<Attachments>,
          PowerPlain<Attachments>,
          PowerHit<Attachments>,
          PowerSneaking<Attachments>,
          PowerHurt<Attachments>,
          PowerHitTaken<Attachments>,
          PowerBowShoot<Attachments> {
    ...
    public PowerResult<Void> fire(Attachments power, Player player, ItemStack stack, Event event) {
      Set<RPGItem> allow =
          (power.getAllowedItems() == null || power.getAllowedItems().isEmpty())
              ? null
              : power.getAllowedItems().stream()
                  .flatMap(
                      s -> {
                        try {
    ...
    public PowerResult<Double> takeHit(
        Attachments power, Player target, ItemStack stack, double damage, EntityDamageEvent event) {
    ...
    public PowerResult<Void> hurt(
        Attachments power, Player target, ItemStack stack, EntityDamageEvent event) {
      if (!power.isRequireHurtByEntity() || event instanceof EntityDamageByEntityEvent) {
    ...
    public PowerResult<Void> leftClick(
        Attachments power, Player player, ItemStack stack, PlayerInteractEvent event) {
    ...
    public PowerResult<Float> bowShoot(
        Attachments power, Player player, ItemStack stack, EntityShootBowEvent event) {

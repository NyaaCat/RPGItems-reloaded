...
public class Projectiles extends BasePower {
  ...
  private final Cache<UUID, Integer> burstTask =
      CacheBuilder.newBuilder().expireAfterAccess(1, TimeUnit.MINUTES).concurrencyLevel(2).build();
  ...
  public static class Impl
    ...
    public PowerResult<Double> takeHit(
        Projectiles power, Player target, ItemStack stack, double damage, EntityDamageEvent event) {
        ...
        if (power.getFiringLocation().equals(FiringLocation.TARGET) && castLocation != null) {
          ...
          Location fireLocation =
              CastUtils.parseFiringLocation(
                  targetLocation, y_axis, player.getEyeLocation(), roundedConeInfo);
              ...
              world.spawn(
                  ...
                  armorStand -> {
                    ...
                    fireLocation1.setDirection(
                        targetLocation.toVector().subtract(fireLocation.toVector()));
    ...
    @Override
    public PowerResult<Void> fire(
        Projectiles power, Player player, ItemStack stack, LivingEntity entity, Double value) {
      CastUtils.CastLocation castLocation = null;
      if (power.getFiringLocation().equals(FiringLocation.TARGET)) {
        castLocation =
            CastUtils.rayTrace(

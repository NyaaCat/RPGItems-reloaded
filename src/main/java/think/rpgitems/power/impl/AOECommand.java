...
public class AOECommand extends Command {
  ...
  /**
   * Type of targets. Can be `entity` `player` `mobs` now entity: apply the command to every {@link
   * LivingEntity} in range player: apply the command to every {@link Player} in range mobs: apply
   * the command to every {@link LivingEntity} except {@link Player}in range
   */
  ...
  public static class Impl
    ...
    private PowerResult<Void> fire(
        AOECommand power, Player player, ItemStack stack, List<LivingEntity> entitiesInCone) {
    ...
    public PowerResult<Void> beamEnd(
        AOECommand power, Player player, ItemStack stack, Location location, BeamEndEvent event) {
      int r = power.getR();
      List<LivingEntity> nearbyEntities =
          getNearbyEntities(power, location, player, r).stream()
              .filter(entity -> entity instanceof LivingEntity)

...
public class AOE extends BasePower {
  ...
  public static class Impl
    ...
    private PowerResult<Void> fire(
      ...
      PotionEffect effect =
          new PotionEffect(power.getType(), power.getDuration(), power.getAmplifier() - 1);
      ...
      getLivingEntitiesInConeSorted(collect, center.toVector(), power.getAngle(), subtract).stream()
          .filter(
              entity -> {
          ...
          .forEach(
              ent -> {
                ...
                if (ent != null
                    && Objects.equals(ent.getLocation().getWorld(), center.getWorld())
    ...
    public PowerResult<Void> hitBlock(
        AOE power, Player player, ItemStack stack, Location location, BeamHitBlockEvent event) {
      Location center = event.getLocation();
      return fire(
          power, center, player, getNearbyEntities(power, center, player, power.getRange()));
    }
    ...
    public PowerResult<Void> fire(
        AOE power, Player player, ItemStack stack, LivingEntity entity, @Nullable Double value) {
      return fire(
          power,
    ...
    public PowerResult<Void> fire(AOE power, Player player, ItemStack stack, Location location) {
      return fire(
          power, location, player, getNearbyEntities(power, location, player, power.getRange()));
    }
  }
}

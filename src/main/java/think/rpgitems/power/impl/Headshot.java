...
public class Headshot extends BasePower {
    ...
    @Property public boolean particleEnemy = true;

    @Property public boolean soundSelf = true;

    @Property public boolean soundEnemy = false;
    ...
    public static class Impl implements PowerHit<Headshot>, PowerBeamHit<Headshot> {
        ...
        public PowerResult<Double> hit(
            ...
            return check(
                    ...
                    damage,
                    damager.getVelocity(),
                    damager.getBoundingBox(),
                    entity.getBoundingBox(),
                    event);
        }

        private PowerResult<Double> check(
                Headshot power,
                Player player,
                LivingEntity entity,
                ItemStack stack,
                double damage,
                Vector velocity,
                BoundingBox damagerOrigBb,
                BoundingBox entityBb,
                EntityDamageByEntityEvent event) {
            double maxAxis =
                    Math.max(
                            entityBb.getHeight(),
                            Math.max(entityBb.getWidthX(), entityBb.getWidthZ()));
            double minAxis =
                    Math.min(
                            entityBb.getHeight(),
                            Math.min(entityBb.getWidthX(), entityBb.getWidthZ()));
            ...
            if (hs) {
                Context.instance()
                        .putExpiringSeconds(player.getUniqueId(), "headshot.target", entity, 3);
                if (power.isSoundSelf()) {
                    player.getWorld()
                            .playSound(player.getLocation(), Sound.BLOCK_ANVIL_PLACE, 1, 3);
                }
                if (power.isSoundEnemy()) {
                    entity.getWorld()
                            .playSound(entity.getLocation(), Sound.ENTITY_GHAST_HURT, 1, 3);
                }
        ...
        public PowerResult<Double> hitEntity(
            ...
            EntityDamageByEntityEvent fake =
                    new EntityDamageByEntityEvent(
                            player, player, EntityDamageEvent.DamageCause.CUSTOM, damage);
            PowerResult<Double> check =
                    check(
                            power,
                            player,
                            entity,
                            ...
                            damage,
                            event.getVelocity(),
                            event.getBoundingBox().expand(0.5),
                            entity.getBoundingBox(),
                            fake);

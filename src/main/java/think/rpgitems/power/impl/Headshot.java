...
public class Headshot extends BasePower {
  ...
  public static class Impl implements PowerHit<Headshot>, PowerBeamHit<Headshot> {
    ...
    private PowerResult<Double> check(
        ...
        EntityDamageByEntityEvent event) {
      double maxAxis =
          Math.max(entityBb.getHeight(), Math.max(entityBb.getWidthX(), entityBb.getWidthZ()));
      double minAxis =
          Math.min(entityBb.getHeight(), Math.min(entityBb.getWidthX(), entityBb.getWidthZ()));
    ...
    public PowerResult<Void> beamEnd(
        Headshot power, Player player, ItemStack stack, Location location, BeamEndEvent event) {
      return PowerResult.fail();
    }
  }
}

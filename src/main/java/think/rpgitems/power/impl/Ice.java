 ...
 * Power ice.
 *
 * <p>The ice power will fire an ice block on right click which will then create a box of ice on
 * impact, the ice will slowly remove itself.
 */
@Meta(
        defaultTrigger = "RIGHT_CLICK",
        withSelectors = true,
        generalInterface = PowerPlain.class,
        implClass = Ice.Impl.class)
public class Ice extends BasePower {
    ...
    public static class Impl
            implements PowerRightClick<Ice>,
                    PowerLeftClick<Ice>,
                    PowerSprint<Ice>,
                    PowerSneak<Ice>,
                    PowerPlain<Ice>,
        ...
        public PowerResult<Void> rightClick(
                Ice power, final Player player, ItemStack stack, PlayerInteractEvent event) {
                    ...
                    new BukkitRunnable() {
                        public void run() {
                            ...
                            List<Entity> entities =
                                    getNearbyEntities(power, block.getLocation(), player, 1);
                                    ...
                                    for (int y = -1; y < 3; y++) {
                                        for (int z = -1; z < 2; z++) {
                                            ...
                                            if (!b.getType().isSolid()
                                                    && !(b.getType() == Material.PLAYER_HEAD
                                                            || b.getType()
                                                                    == Material.PLAYER_WALL_HEAD)) {
                                                changedBlocks.put(
                                                        b.getLocation(), b.getBlockData());
                                ...
                                (new BukkitRunnable() {
                                            ...
                                            public void run() {
                                                for (int i = 0; i < 4; i++) {
                                                    ...
                                                    int index =
                                                            random.nextInt(changedBlocks.size());
                                                    BlockData data =
                                                            changedBlocks.values()
                                                                    .toArray(new BlockData[0])[
                                                                    index];
                                                    Location position =
                                                            changedBlocks.keySet()
        ...
        public PowerResult<Void> leftClick(
                Ice power, Player player, ItemStack stack, PlayerInteractEvent event) {
            return fire(power, player, stack);
        }

        @Override
        public PowerResult<Float> bowShoot(
                Ice power, Player player, ItemStack stack, EntityShootBowEvent event) {
            return fire(power, player, stack).with(event.getForce());
        }

        @Override
        public PowerResult<Void> sneak(
                Ice power, Player player, ItemStack stack, PlayerToggleSneakEvent event) {
            return fire(power, player, stack);
        }

        @Override
        public PowerResult<Void> sprint(
                Ice power, Player player, ItemStack stack, PlayerToggleSprintEvent event) {
            return fire(power, player, stack);
        }
    }
}

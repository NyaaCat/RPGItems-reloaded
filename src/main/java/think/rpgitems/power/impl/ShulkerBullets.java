...
public class ShulkerBullets extends BasePower {
  ...
  public static class Impl
    ...
    public PowerResult<Void> fire(ShulkerBullets power, Player player, ItemStack stack) {
      ...
      org.bukkit.entity.ShulkerBullet bullet =
          player.launchProjectile(
              org.bukkit.entity.ShulkerBullet.class, player.getEyeLocation().getDirection());
      ...
      if (!entities.isEmpty()) {
        Context.instance()
            .putExpiringSeconds(player.getUniqueId(), "shulkerbullet.target", entities.get(0), 3);
    ...
    public PowerResult<Void> sneak(
        ShulkerBullets power, Player player, ItemStack stack, PlayerToggleSneakEvent event) {
      return fire(power, player, stack);
    }

    @Override
    public PowerResult<Void> sprint(
        ShulkerBullets power, Player player, ItemStack stack, PlayerToggleSprintEvent event) {
      return fire(power, player, stack);
    }

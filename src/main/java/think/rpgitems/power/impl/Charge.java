...
public class Charge extends BasePower {
    ...
    public static class Impl implements PowerHit<Charge> {
        @Override
        public PowerResult<Double> hit(Charge power, Player player, ItemStack stack, LivingEntity entity, double damage, EntityDamageByEntityEvent event) {
            if (!player.isSprinting())
                return PowerResult.noop();
            double originDamage = damage;
            damage = damage * (1 + power.getPercentage() / 100.0);
            damage = damage + damage * (player.getAttribute(Attribute.GENERIC_MOVEMENT_SPEED).getValue() / 0.13 - 1) * (power.getSpeedPercentage() / 100.0);

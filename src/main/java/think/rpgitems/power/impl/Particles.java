...
public class Particles extends BasePower {
  ...
  private Object getData() {
    if (data != null || getParticle().getDataType().equals(Void.class)) {
      ...
      data =
          new org.bukkit.Particle.DustOptions(Color.fromRGB(getDustColor()), (float) getDustSize());
    }
    return data;
  }
  ...
  public static class Impl
    ...
    public PowerResult<Double> takeHit(
        Particles power, Player target, ItemStack stack, double damage, EntityDamageEvent event) {
    ...
    public PowerResult<Void> beamEnd(
        Particles power, Player player, ItemStack stack, Location location, BeamEndEvent event) {
      int delay = power.getDelay();
      new BukkitRunnable() {
        @Override
        public void run() {

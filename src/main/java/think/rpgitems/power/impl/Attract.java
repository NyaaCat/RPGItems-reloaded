 ...
 * Power attract.
 *
 * <p>Pull around mobs in {@link #radius radius} to player. Moving the mobs with max speed of {@link
 * #maxSpeed maxSpeed}
 */
...
public class Attract extends BasePower {
  ...
  @Property public int duration = 5;
  @Property public int attractingTickCost = 0;
  @Property public int attractingEntityTickCost = 0;

  @Property public boolean attractPlayer;
  ...
  public static class Impl
      implements PowerTick<Attract>,
          PowerLeftClick<Attract>,
          PowerRightClick<Attract>,
          PowerPlain<Attract>,
          PowerSneaking<Attract>,
          PowerHurt<Attract>,
          PowerHitTaken<Attract>,
          PowerBowShoot<Attract>,
          PowerBeamHit<Attract>,
          PowerProjectileHit<Attract>,
          PowerLivingEntity<Attract>,
          PowerLocation<Attract> {

    @Override
    public PowerResult<Double> takeHit(
        Attract power, Player target, ItemStack stack, double damage, EntityDamageEvent event) {
    ...
    public PowerResult<Void> fire(Attract power, Player player, ItemStack stack) {
      ...
      return fire(
          ...
          location,
          stack,
          () -> getNearbyEntities(power, finalLocation, player, power.getRadius()));
    }
    ...
    private PowerResult<Void> attract(
        Attract power,
        Player player,
        Location location,
        ItemStack stack,
      ...
      for (Entity e : entities) {
        if (e instanceof LivingEntity && (power.isAttractPlayer() || !(e instanceof Player))) {
    ...
    public PowerResult<Void> hurt(
        Attract power, Player target, ItemStack stack, EntityDamageEvent event) {
      if (!power.isRequireHurtByEntity() || event instanceof EntityDamageByEntityEvent) {
    ...
    private PowerResult<Void> attract(Attract power, Player player, ItemStack stack) {
      ...
      return attract(
          ...
          location,
          stack,
          () -> getNearbyEntities(power, player.getLocation(), player, power.getRadius()));
    }
    ...
    public PowerResult<Void> leftClick(
        Attract power, Player player, ItemStack stack, PlayerInteractEvent event) {
    ...
    public PowerResult<Float> bowShoot(
        Attract power, Player player, ItemStack stack, EntityShootBowEvent event) {
    ...
    public PowerResult<Double> hitEntity(
      ...
      return fire(
              ...
              location,
              stack,
              () -> getNearbyEntities(power, location, player, power.getRadius()))
          .with(damage);
    }

    @Override
    public PowerResult<Void> hitBlock(
        Attract power, Player player, ItemStack stack, Location location, BeamHitBlockEvent event) {
      return fire(
          power,
          player,
          location,
    ...
    public PowerResult<Void> beamEnd(
        Attract power, Player player, ItemStack stack, Location location, BeamEndEvent event) {
      return fire(
          power,
          player,
          location,
    ...
    public PowerResult<Void> projectileHit(
        Attract power, Player player, ItemStack stack, ProjectileHitEvent event) {
    ...
    @Override
    public PowerResult<Void> fire(
        Attract power, Player player, ItemStack stack, Location location) {
      return fire(
          power,
          player,
          location,

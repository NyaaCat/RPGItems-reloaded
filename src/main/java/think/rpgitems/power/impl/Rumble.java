 ...
 * Power rumble.
 *
 * <p>The rumble power sends a shockwave through the ground and sends any hit entities flying with
 * power {@link #power}. The wave will travel {@link #distance} blocks.
 */
...
public class Rumble extends BasePower {
  ...
  public static class Impl
      implements PowerRightClick<Rumble>,
          PowerLeftClick<Rumble>,
          PowerSneak<Rumble>,
          PowerSprint<Rumble>,
          PowerPlain<Rumble>,
          PowerBowShoot<Rumble>,
          PowerLivingEntity<Rumble> {
    @Override
    public PowerResult<Void> leftClick(
        Rumble power, Player player, ItemStack stack, PlayerInteractEvent event) {
    ...
    private PowerResult<Void> fire(
        Rumble power, Player player, Location location, Vector direction) {
      direction.setY(0);
      direction.normalize();
      BukkitRunnable task =
          new BukkitRunnable() {
              ...
              if (hit) {
                ...
                for (Entity e : near) {
                  if (e != player) {
                    if (e instanceof ItemFrame || e instanceof Painting) {
                      e.setMetadata(
                          ...
                          new FixedMetadataValue(
                              RPGItems.plugin,
                              null)); // Add metadata to protect hanging entities from the explosion
                      continue;
                    }
                    ...
                    if (power.getDamage() > 0) {
                      ...
                      Context.instance()
                          .putTemp(player.getUniqueId(), OVERRIDING_DAMAGE, power.getDamage());
                ...
                location
                    ...
                    .createExplosion(
                        ...
                        power.getPower(),
                        false,
                        false); // Trigger the explosion after all hanging entities have been
                // protected
                cancel();
                return;
              }
    ...
    public PowerResult<Float> bowShoot(
        Rumble power, Player player, ItemStack stack, EntityShootBowEvent event) {
      return fire(power, player, stack).with(event.getForce());
    }

    @Override
    public PowerResult<Void> sneak(
        Rumble power, Player player, ItemStack stack, PlayerToggleSneakEvent event) {
      return fire(power, player, stack);
    }

    @Override
    public PowerResult<Void> sprint(
        Rumble power, Player player, ItemStack stack, PlayerToggleSprintEvent event) {
      return fire(power, player, stack);
    }

    @Override
    public PowerResult<Void> fire(
        Rumble power, Player player, ItemStack stack, LivingEntity entity, @Nullable Double value) {

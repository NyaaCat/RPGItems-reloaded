    ...
    public PowerResult<Void> fire(Torch power, final Player player, ItemStack stack) {
      ...
      final FallingBlock block =
          player
              .getWorld()
              .spawnFallingBlock(
                  player.getLocation().add(0, 1.8, 0), Material.TORCH.createBlockData());
            ...
            public void run() {
              ...
              if (block.isDead()) {
                ...
                if (block.getLocation().getBlock().getType().equals(Material.TORCH))
                  block.setMetadata(
                      "RPGItems.Torch", new FixedMetadataValue(RPGItems.plugin, null));
                cancel();
                      ...
                      if (b.getType().equals(Material.AIR) && random.nextInt(100) < 20) {
                        ...
                        if (faces.size() > 0) {
                          ...
                          b.setMetadata(
                              "RPGItems.Torch", new FixedMetadataValue(RPGItems.plugin, null));
                          b.setType(
                              o == BlockFace.DOWN ? Material.TORCH : Material.WALL_TORCH, false);
                          if (o != BlockFace.DOWN) {

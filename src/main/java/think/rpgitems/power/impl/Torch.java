 ...
 * Power torch.
 *
 * <p>The torch power will shoots torches to light up an area.
 */
@Meta(
        defaultTrigger = "RIGHT_CLICK",
        generalInterface = PowerPlain.class,
        implClass = Torch.Impl.class)
public class Torch extends BasePower {
    ...
    public static class Impl
            implements PowerPlain<Torch>,
                    PowerLeftClick<Torch>,
                    PowerRightClick<Torch>,
                    PowerBowShoot<Torch> {

        @Override
        public PowerResult<Void> leftClick(
                Torch power, Player player, ItemStack stack, PlayerInteractEvent event) {
                        ...
                        public void run() {
                            ...
                            if (block.isDead()) {
                                ...
                                if (block.getLocation().getBlock().getType().equals(Material.TORCH))
                                    block.setMetadata(
                                            "RPGItems.Torch",
                                            new FixedMetadataValue(RPGItems.plugin, null));
                                cancel();
                                final HashMap<Location, Material> changedBlocks = new HashMap<>();
                                final HashMap<Location, BlockData> changedBlockData =
                                        new HashMap<>();
                                for (int x = -2; x <= 2; x++) {
                                    for (int y = -2; y <= 3; y++) {
                                        for (int z = -2; z <= 2; z++) {
                                            ...
                                            public void run() {
                                                if (changedBlocks.isEmpty()) {
                                                    ...
                                                    block.removeMetadata(
                                                            "RPGItems.Torch", RPGItems.plugin);
                                                    if (block.getLocation().getBlock().getType()
                                                            == Material.TORCH) {
                                                        block.getLocation()
                                                                .getBlock()
        ...
        public PowerResult<Float> bowShoot(
                Torch power, Player player, ItemStack stack, EntityShootBowEvent event) {

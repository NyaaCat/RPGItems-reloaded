...
@Meta(
        defaultTrigger = {"RIGHT_CLICK", "TICK"},
        generalInterface = PowerPlain.class,
        implClass = ParticleBarrier.Impl.class)
public class ParticleBarrier extends BasePower {

    private static final Cache<UUID, Double> barriers =
            CacheBuilder.newBuilder().expireAfterAccess(1, TimeUnit.MINUTES).build();
    private static final Cache<UUID, UUID> barrierSources =
            CacheBuilder.newBuilder().expireAfterAccess(1, TimeUnit.MINUTES).build();
    private static final Cache<UUID, Pair<Long, Double>> energys =
            CacheBuilder.newBuilder().expireAfterAccess(10, TimeUnit.MINUTES).build();
    private static AtomicInteger rc = new AtomicInteger(0);
    private static Listener event;
    @Property public double energyPerBarrier = 40;
    @Property public double barrierHealth = 40;
    @Property public double energyDecay = 1.5;
    @Property public double energyPerLevel = 10;
    @Property public boolean projected = false;
                    ...
                    new Listener() {
                        ...
                        public void onEntityDamage(EntityDamageEvent entityDamageEvent) {
                            ...
                            double energyGain =
                                    Math.min(
                                            getEnergyPerBarrier(),
                                            damage * getEnergyPerBarrier() / getBarrierHealth());
                            ...
                            double currentEnergy =
                                    last && pair.getValue() > 0 ? pair.getValue() : 0;
                            double energy =
                                    currentEnergy
                                            - (currentTime - lastTime) * getEnergyDecay() / 1000
                                            + energyGain;
                            getEnergys()
                                    .put(source, new Pair<>(currentTime, Math.min(energy, 100.0d)));
                        }
                    };
    ...
    public static class Impl
            implements PowerPlain<ParticleBarrier>,
                    PowerRightClick<ParticleBarrier>,
                    PowerLeftClick<ParticleBarrier>,
                    PowerTick<ParticleBarrier>,
                    PowerBowShoot<ParticleBarrier> {

        @Override
        public PowerResult<Void> leftClick(
                ParticleBarrier power, Player player, ItemStack stack, PlayerInteractEvent event) {
        ...
        public PowerResult<Void> fire(ParticleBarrier power, Player player, ItemStack stack) {
            if (!power.isProjected()) {
                ...
                Optional<LivingEntity> optionalPlayer =
                        livingEntities.stream()
                                .min(
            ...
            new BukkitRunnable() {
                ...
                public void run() {
                    ...
                    if (dur > 0 && barrierRemain != null && barrierRemain > 0) {
                        ...
                        eyeLocation
                                .getWorld()
                                .playSound(eyeLocation, Sound.BLOCK_GLASS_BREAK, 1.0f, 0.5f);
                        cancel();
                        return;
                    }
                    Location eyeLocation = target.getEyeLocation();
        ...
        public PowerResult<Float> bowShoot(
                ParticleBarrier power, Player player, ItemStack stack, EntityShootBowEvent event) {
        ...
        public PowerResult<Void> tick(ParticleBarrier power, Player player, ItemStack stack) {
            ...
            double energy =
                    pair.getValue()
                            - (currentTimeMillis - pair.getKey()) * power.getEnergyDecay() / 1000;

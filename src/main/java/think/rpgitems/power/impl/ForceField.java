...
public class ForceField extends BasePower {
                  ...
                  for (int i = h; i >= l; i--) {
                    for (Location l : circlePoints) {
                      ...
                      if (!wasBarrier.contains(l)
                          && w.getBlockAt(l).getType() == Material.BARRIER) {
  ...
  public static class Impl
    ...
    public PowerResult<Void> fire(ForceField power, Player player, ItemStack stack) {
      ...
      buildWallTask tsk =
          new buildWallTask(w, circlePoints(w, x, z, power.getRadius(), l), l, h, power.getTtl());
    ...
    public PowerResult<Double> takeHit(
        ForceField power, Player target, ItemStack stack, double damage, EntityDamageEvent event) {

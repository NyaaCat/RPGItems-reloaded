    ...
    public PowerResult<Void> rightClick(
      ...
      List<LivingEntity> entities =
          getLivingEntitiesInCone(
              getNearestLivingEntities(
                  power, player.getEyeLocation(), player, power.getMaxDistance(), 0),
      ...
      for (LivingEntity entity : entities) {
        if (entity.isValid()
          ...
          player
              .getWorld()
              .playSound(player.getLocation(), Sound.BLOCK_DISPENSER_DISPENSE, 1.0F, 1.0F);
          ...
          new BukkitRunnable() {
            ...
            public void run() {
              if (getTicks() >= power.getMaxTicks()
                ...
                if (!player.getPassengers().isEmpty()
                    && player.getPassengers().get(0).getUniqueId().equals(entity.getUniqueId())) {

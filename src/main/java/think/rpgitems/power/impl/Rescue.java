 ...
 * Power rescue.
 *
 * <p>The rescue power teleports the user to spawn (or to their bed when {@link #useBed} is active)
 * or rescue them in place when {@link #inPlace} when their health gets below the {@link
 * #healthTrigger} while in combat with an enemy or when they takes a damage greater than {@link
 * #damageTrigger}
 */
@SuppressWarnings("WeakerAccess")
@Meta(
    defaultTrigger = {"DYING"},
    implClass = Rescue.Impl.class)
public class Rescue extends BasePower {
  private static Cache<UUID, Long> rescueTime =
  ...
  @Property public double damageTrigger = 1024;
  ...
  public String displayText() {
    return I18n.formatDefault(
        "power.rescue.display", ((double) getHealthTrigger()) / 2, (double) 0 / 20d);
  }
  ...
  public static class Impl implements PowerHurt<Rescue>, PowerHitTaken<Rescue> {
    ...
    public PowerResult<Void> hurt(
        Rescue power, Player target, ItemStack stack, EntityDamageEvent event) {
    ...
    private PowerResult<Double> rescue(
        Rescue power, Player target, ItemStack stack, EntityDamageEvent event, boolean canceled) {
    ...
    public PowerResult<Double> takeHit(
        Rescue power, Player target, ItemStack stack, double damage, EntityDamageEvent event) {
      double health = target.getHealth() - event.getFinalDamage();
      if (health > power.getHealthTrigger() && event.getFinalDamage() < power.getDamageTrigger())

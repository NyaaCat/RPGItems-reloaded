...
public class Dummy extends BasePower {
  ...
  public static class Impl
    ...
    public PowerResult<Void> fire(
      ...
      if (!checkAndSetCooldown(
          ...
          (power.isGlobalCooldown() ? "" : (power.getItem().getUid() + "."))
              + power.getCooldownKey())) return PowerResult.of(power.getCooldownResult());
    ...
    public PowerResult<Double> takeHit(
        Dummy power, Player target, ItemStack stack, double damage, EntityDamageEvent event) {
    ...
    public PowerResult<Void> hitBlock(
        Dummy power, Player player, ItemStack stack, Location location, BeamHitBlockEvent event) {
      return fire(power, player, stack);
    }

    @Override
    public PowerResult<Void> beamEnd(
        Dummy power, Player player, ItemStack stack, Location location, BeamEndEvent event) {
      return fire(power, player, stack);
    }
  }
}

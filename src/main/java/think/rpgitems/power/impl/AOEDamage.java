...
public class AOEDamage extends BasePower {
  ...
  public static class Impl
            ...
            if (power.getFiringLocation().equals(FiringLocation.TARGET)) {
              ...
              ent =
                  getNearestLivingEntities(
                      power, targetLocation, player, power.getRange(), power.getMinrange());
            } else {
              nearbyEntities =
                  getNearestLivingEntities(
                      power, player.getLocation(), player, power.getRange(), power.getMinrange());
              ent =
    ...
    private PowerResult<Void> fire(
        AOEDamage power, Player player, ItemStack stack, Supplier<List<LivingEntity>> supplier) {
      Context.instance()
          .putTemp(player.getUniqueId(), DAMAGE_SOURCE, power.getNamespacedKey().toString());
    ...
    public PowerResult<Double> takeHit(
        AOEDamage power, Player target, ItemStack stack, double damage, EntityDamageEvent event) {
    ...
    public PowerResult<Void> beamEnd(
        AOEDamage power, Player player, ItemStack stack, Location location, BeamEndEvent event) {
      int range = power.getRange();
      return fire(power, player, stack, () -> getNearbyEntities(power, player, location, range));
    }

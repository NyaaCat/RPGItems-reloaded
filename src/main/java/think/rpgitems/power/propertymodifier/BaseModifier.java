...
public abstract class BaseModifier<T> extends BasePropertyHolder implements Modifier<T> {
  ...
  public void init(PersistentDataContainer section) {
    ...
    Map<String, Pair<Method, PropertyInstance>> properties =
        PowerManager.getProperties(this.getClass());
          ...
          if (Collection.class.isAssignableFrom(field.getType())) {
            ...
            if (Set.class.isAssignableFrom(field.getType())) {
              set(
                  ...
                  name,
                  (String)
                      c.stream().map(Object::toString).sorted().collect(Collectors.joining(",")));
            } else {
              set(
                  section,
                  name,
                  (String) c.stream().map(Object::toString).collect(Collectors.joining(",")));
            }
  ...
  public boolean match(Power orig, PropertyInstance propertyInstance) {
    ...
    if (!Strings.isNullOrEmpty(targetPower)
        && !orig.getNamespacedKey().equals(PowerManager.parseKey(targetPower))) {
      return false;
    }

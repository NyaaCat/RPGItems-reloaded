...
public class Utils {
    ...
    private static LoadingCache<String, List<String>> permissionCache =
    ...
    public static List<Entity> getNearbyEntities(
        ...
        power.getItem().getMarkers().stream()
                .filter(pow -> pow instanceof Selector)
                .forEach(
                        selector -> {
     ...
     */
    public static List<Entity> getNearbyEntities(
            Power power, Location l, Player player, double radius) {
    ...
    public static List<LivingEntity> getNearestLivingEntities(
            Power power, Location l, Player player, double radius, double min) {
     ...
     */
    public static List<LivingEntity> getLivingEntitiesInCone(
            List<LivingEntity> entities,
            org.bukkit.util.Vector startPos,
            double degrees,
    ...
    public static List<LivingEntity> getLivingEntitiesInConeSorted(
            List<LivingEntity> entities,
            org.bukkit.util.Vector startPos,
            double degrees,
            org.bukkit.util.Vector direction) {
    ...
    public static float getAngleBetweenVectors(
            org.bukkit.util.Vector v1, org.bukkit.util.Vector v2) {
    ...
    public static boolean checkCooldown(
            Power power, Player player, long cdTicks, boolean showWarn, boolean showPower) {
        return checkAndSetCooldown(
                ...
                showWarn,
                showPower,
                "cooldown." + power.getItem().getUid() + "." + power.getNamespacedKey().toString());
    }

    public static boolean checkCooldown(
            PowerChain power, Player player, long cdTicks, boolean showWarn, boolean showPower) {
        return checkAndSetCooldown(
                power,
                player,
                cdTicks,
                showWarn,
                showPower,
                "cooldown." + power.getItem().getUid() + "." + power.getChainId());
    }

    public static boolean checkAndSetCooldown(
            PowerChain power,
            Player player,
            long cooldownTick,
            boolean showWarn,
    ...
    private static boolean checkCooldown(
            ...
            boolean showWarn,
            String key,
            BiConsumer<Long, Long> cdWarning) {
        long cooldown;
    ...
    private static void showCDWarning(
            Player player, Power power, long cooldown, long nowTime, boolean showPower) {
        I18n i18n = I18n.getInstance(player.getLocale());
        if (showPower) {
            player.sendMessage(
                    i18n.format(
                            "message.cooldown.power",
                            ((double) (cooldown - nowTime)) / 50d / 20d,
                            power.getItem().getDisplayName(),
                            power.getLocalizedName(player)));
        } else {
            player.sendMessage(
                    i18n.format(
                            "message.cooldown.general",
                            ((double) (cooldown - nowTime)) / 50d / 20d,
                            power.getItem().getDisplayName()));
        }
    }
    ...
    private static void showCDWarning(
            Player player, PowerChain power, long cooldown, long nowTime, boolean showPower) {
        I18n i18n = I18n.getInstance(player.getLocale());
        if (showPower) {
            player.sendMessage(
                    i18n.format(
    ...
    public static void saveProperty(
            PropertyHolder p, ConfigurationSection section, String property, Field field)
    ...
    public static String getProperty(PropertyHolder p, String property, Field field) {
        ...
        try {
            ...
            RPGItems.plugin
                    ...
                    .log(
                            Level.WARNING,
                            "Error getting property " + property + " from " + field + " in " + p,
                            e);
            return null;
        }
    ...
    public static void setPowerPropertyUnchecked(
            CommandSender sender, PropertyHolder power, Field field, String value) {
            ...
            if (st != null) {
                ...
                if (field.getType().equals(int.class) || field.getType().equals(Integer.class)) {
                    ...
                    try {
                        ...
                        throw new AdminCommands.CommandException(
                                "internal.error.bad_double", value);
                    }
                    ...
                    try {
                        ...
                        throw new AdminCommands.CommandException(
                                "internal.error.bad_enum",
                                field.getName(),
                                Stream.of(field.getType().getEnumConstants())
                                        .map(Object::toString)
                                        .collect(Collectors.joining(", ")));
                    }
                    ...
                    Stream<String> values =
                            Arrays.stream(valueStrs).filter(s -> !s.isEmpty()).map(String::trim);
                    if (field.getType().equals(List.class)) {
                        if (listArg.isEnum()) {
                            ...
                            List<Integer> list =
                                    values.map(Integer::parseInt).collect(Collectors.toList());
                            field.set(power, list);
                        } else if (listArg.equals(Double.class)) {
                            List<Double> list =
                                    values.map(Double::parseDouble).collect(Collectors.toList());
                            field.set(power, list);
                        } else {
                            throw new AdminCommands.CommandException(
                        ...
                        if (listArg.isEnum()) {
                            ...
                            Set<Trigger> set =
                                    Trigger.getValid(values.collect(Collectors.toList()), ignored);
                            if (!ignored.isEmpty()) {
                                new Message(
                                                I18n.formatDefault(
                                                        "message.power.ignored_trigger",
                                                        String.join(", ", ignored),
                                                        power.getName(),
                                                        power.getItem().getName()))
                                        .send(sender);
                            }
                            field.set(power, set);
                        } else if (listArg.equals(Integer.class)) {
                            Set<Integer> list =
                                    values.map(Integer::parseInt).collect(Collectors.toSet());
                            field.set(power, list);
                        } else if (listArg.equals(Double.class)) {
                            Set<Double> list =
                                    values.map(Double::parseDouble).collect(Collectors.toSet());
                            field.set(power, list);
                        } else {
                            throw new AdminCommands.CommandException(
                                    "internal.error.command_exception");
                        ...
                        if (value.startsWith("minecraft:")) {
                            enchantment =
                                    Enchantment.getByKey(
                                            NamespacedKey.minecraft(value.split(":", 2)[1]));
                        } else {
                            enchantment =
                                    Enchantment.getByKey(
                                            new NamespacedKey(
                    ...
                    if (enchantment == null) {
                        enchantment =
                                Arrays.stream(Enchantment.class.getDeclaredFields())
                                        .parallel()
                                        ...
                                        .map(
                                                f -> {
                                                    try {
                                                        ...
                                                        throw new AdminCommands.CommandException(
                                                                "message.error.invalid_enchant", e);
                                                    }
                                                })
                                        .orElse(null);
                    }
                    ...
                    throw new AdminCommands.CommandException(
                            "message.error.invalid_command_arg", power.getName(), field.getName());
                }
            }
        } catch (IllegalAccessException e) {
    ...
    public static void setPowerProperty(
            CommandSender sender, PropertyHolder power, Field field, String value)
            throws IllegalAccessException {
    ...
    public static LazyFunction scoreBoard(Player player) {
        return new LazyFunction() {
            ...
            public Expression.LazyNumber lazyEval(List<Expression.LazyNumber> lazyParams) {
                Objective objective =
                        player.getScoreboard().getObjective(lazyParams.get(0).getString());
                if (objective == null) {
    ...
    public static LazyFunction context(Player player) {
        return new LazyFunction() {
            ...
            public Expression.LazyNumber lazyEval(List<Expression.LazyNumber> lazyParams) {
                Object obj =
                        Context.instance().get(player.getUniqueId(), lazyParams.get(0).getString());
                if (obj == null) {
                    return lazyParams.get(1);
                }
                return new Expression.LazyNumber() {
    ...
    public static double eval(
        ...
        ex.and("damage", BigDecimal.valueOf(damage))
                ...
                .and(
                        "playerPitch",
                        Utils.lazyNumber(() -> (double) player.getLocation().getPitch()))
        ...
        if (damager != null) {
            ...
            Entity entity = ent;
            ex.and("damagerType", damager.getType().name())
                    .and("isDamageByProjectile", byProjectile ? BigDecimal.ONE : BigDecimal.ZERO)
                    .and(
                            "damagerTicksLived",
                            Utils.lazyNumber(() -> (double) damager.getTicksLived()))
                    .and(
                            "distance",
                            Utils.lazyNumber(
                    ...
                    .and(
                            "entityYaw",
                            Utils.lazyNumber(() -> (double) entity.getLocation().getYaw()))
                    .and(
                            "entityPitch",
                            Utils.lazyNumber(() -> (double) entity.getLocation().getPitch()))
    ...
    public static <T extends Weightable> T weightedRandomPick(Collection<T> collection) {
        int sum = collection.stream().mapToInt(Weightable::getWeight).sum();
        if (sum == 0) {
            return collection.stream().findAny().orElse(null);
        }
        ThreadLocalRandom random = ThreadLocalRandom.current();
        int selected = random.nextInt(sum);
        Iterator<Pair<T, Integer>> iterator =
                collection.stream().map(t -> new Pair<>(t, t.getWeight())).iterator();
        int count = 0;

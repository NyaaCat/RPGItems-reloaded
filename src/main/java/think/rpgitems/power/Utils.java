...
public class Utils {
  ...
  public static List<Entity> getNearbyEntities(
      Power power, Location l, Player player, double radius, double dx, double dy, double dz) {
    ...
    if (getter != null) {
      ...
      if (Collection.class.isAssignableFrom(field.getType())) {
        ...
        if (Set.class.isAssignableFrom(field.getType())) {
          section.set(
              property, c.stream().map(Object::toString).sorted().collect(Collectors.joining(",")));
        } else {
        ...
        if (field.getType().equals(int.class) || field.getType().equals(Integer.class)) {
          ...
          if (value.equalsIgnoreCase("true") || value.equalsIgnoreCase("false")) {
            ...
            throw new AdminCommands.CommandException(
                "message.error.invalid_option", value, field.getName(), "true, false");
          }
                  ...
                  Enchantment.getByKey(
                      new NamespacedKey(
                          Objects.requireNonNull(
                              Bukkit.getPluginManager().getPlugin(value.split(":", 2)[0])),
                          value.split(":", 2)[1]));
            }
          } else {
    ...
    if (as != null) {
      ...
      if (!Collection.class.isAssignableFrom(field.getType())) {
        if (!acc.contains(value))
          throw new AdminCommands.CommandException(
              "message.error.invalid_option", value, field.getName(), String.join(", ", acc));
      } else {
        String[] valueStrs = value.split(",");
        List<String> values =
  ...
  public static double eval(
      Player player, double damage, EntityDamageEvent event, Entity damager, RPGItem rpgItems) {
    ...
    if (damager != null) {
      ...
      ex.and("damagerType", damager.getType().name())
          ...
          .and(
              "distance",
              Utils.lazyNumber(() -> player.getLocation().distance(entity.getLocation())))

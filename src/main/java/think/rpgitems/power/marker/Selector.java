 ...
 * Power selector.
 *
 * <p>Not a triggerable power. Provide a selector for some AOE power.
 */
@Meta(marker = true)
public class Selector extends BaseMarker {

    private static LoadingCache<String, Map<String, Pair<Integer, Integer>>> scoreCache =
    ...
    private static LoadingCache<String, Pair<Set<String>, Set<String>>> tagCache =
            CacheBuilder.newBuilder()
                    ...
                    .expireAfterAccess(1, TimeUnit.DAYS)
                    .build(CacheLoader.from(Selector::parse));
    private static LoadingCache<String, Set<EntityType>> typeCache =
            CacheBuilder.newBuilder()
                    .concurrencyLevel(1)
                    .expireAfterAccess(1, TimeUnit.DAYS)
    ...
    @Property public Integer count = null;
    /**
     * X-coordinate of reference position, tilde notation is available. Use player's current
     * position if empty
     */
    @Property public String x;
    /**
     * Y-coordinate of reference position, tilde notation is available. Use player's current
     * position if empty
     */
    @Property public String y;
    /**
     * Z-coordinate of reference position, tilde notation is available. Use player's current
     * position if empty
     */
    @Property public String z;
    /** Selects only targets less than r blocks from reference position */
    @Property public Integer r = null;
    /** Selects only targets more than rm blocks from reference position */
    @Property public Integer rm = null;
    /** Selects only targets less than dx blocks in X-axis from reference position */
    @Property public Integer dx = null;
    /** Selects only targets less than dx blocks in Y-axis from reference position */
    @Property public Integer dy = null;
    /** Selects only targets less than dx blocks in Z-axis from reference position */
    @Property public Integer dz = null;
    /**
     * Selecting targets by score(s), According to the following format `score_name:min,max
     * another_score_name:min,max`
     */
    @Property public String score;
    /**
     * Selecting targets by tag(s), According to the following format `MUST_HAVE,!MUST_NOT_HAVE` ``
     * targets without any scoreboard tags `!` targets with any scoreboard tags
     */
    @Property public String tag;
    /**
     * Selecting targets by team(s), According to the following format `MUST_ON,!MUST_NOT_ON` ``
     * targets not on any team `!` targets on any team Only first MUST_ON has effect
     */
    ...
    public void inPlaceFilter(Player p, List<Entity> entities) {
        ...
        List<Entity> ents =
                entities.stream()
                        .filter(
                                entity ->
                                        !Utils.isUtilArmorStand(entity)
                                                && inRadius(entity, ref, r, rm))
                        .collect(Collectors.toList());
        if (Stream.of(dx, dy, dz).anyMatch(Objects::nonNull))
            ents =
                    ents.stream()
                            .filter(entity -> inVolume(entity, ref, dx, dy, dz))
                            .collect(Collectors.toList());
        if (type != null) {
            Set<EntityType> allowType = typeCache.getUnchecked(type);
            ents =
                    ents.stream()
                            .filter(entity -> allowType.contains(entity.getType()))
                            .collect(Collectors.toList());
        }
        if (tag != null) {
        ...
        if (team != null) {
            ...
            ents =
                    ents.stream()
                            .filter(entity -> matchTeam(entity, p.getScoreboard(), t))
                            .collect(Collectors.toList());
        }
        if (score != null) {
            ...
            ents =
                    ents.stream()
                            .filter(entity -> matchScore(entity, p.getScoreboard(), t))
                            .collect(Collectors.toList());
        }
        entities.clear();
    ...
    public static boolean matchTeam(
            Entity e, Scoreboard s, Pair<Set<String>, Set<String>> teamLimit) {
        ...
        return teamLimit.getKey().stream()
                        .findFirst()
                        .map(l -> t != null && l.equals(t.getName()))
                        .orElse(true)
                && (t == null || !teamLimit.getValue().contains(t.getName()));
    }

    public static boolean matchScore(
            Entity e, Scoreboard s, Map<String, Pair<Integer, Integer>> scoreLimit) {
        String name = e.getUniqueId().toString();
        if (e instanceof OfflinePlayer) {
            name = ((OfflinePlayer) e).getName();
        }
        String finalName = name;
        return scoreLimit.entrySet().stream()
                .allMatch(
                        sl -> {
                                    ...
                                    .map(
                                            sco -> {
                                                ...
                                                if (limit.getValue() != null)
                                                    result = sco < limit.getValue();
                                                if (limit.getKey() != null)
                                                    result &= sco >= limit.getKey();
                                                return result;
                                            })
                                    .orElse(false);
                        });

...
public class TriggeringContext {
    ...
    public TriggeringContext(LivingEntity entity, double length) {
        ...
        final LocationVector locationVector =
                selectLocation(eyeLocation, eyeLocation.getDirection(), length);
    }
    ...
    public LocationVector selectLocation(Location start, Vector direction, double length) {
        ...
        final LocationVector locationVector =
                new LocationVector(
                        castLocation.getTargetLocation(), castLocation.getNormalDirection());
     ...
     * @return
     */
    public Location doCast(
            Location fromLocation, Vector towards, double range, RoundedConeInfo roundedConeInfo) {
        // todo cache this
        final CastUtils.CastLocation castLocation =
                CastUtils.rayTrace(fromLocation, towards, range);
        final Location direction =
                new Location(
                        fromLocation.getWorld(), towards.getX(), towards.getY(), towards.getZ());
        return CastUtils.parseFiringLocation(
                castLocation.getTargetLocation(),
                castLocation.getNormalDirection(),
                direction,
                roundedConeInfo);
    }

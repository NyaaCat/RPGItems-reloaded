...
public class I18n extends LanguageRepository {
  ...
  /**
   * add all language items from section into language map recursively overwrite existing items The
   * '&' will be transformed to color code.
   *
   ...
   */
  private static void loadLanguageSection(
      Map<String, String> map,
      ConfigurationSection section,
      String prefix,
      boolean ignoreInternal,
      boolean ignoreNormal) {
    ...
    for (String key : section.getKeys(false)) {
      ...
      if (section.isString(key)) {
        ...
        loadLanguageSection(
            map, section.getConfigurationSection(key), path + ".", ignoreInternal, ignoreNormal);
      }
    }
  }
    ...
    if (stream != null) {
      YamlConfiguration section =
          YamlConfiguration.loadConfiguration(
              new InputStreamReader(stream, Charset.forName("UTF8")));
  ...
  public static I18n getInstance(CommandSender sender) {
    return getInstance(
        (sender instanceof Player) ? ((Player) sender).getLocale() : RPGItems.plugin.cfg.language);
  }
  ...
  public static I18n getInstance(String lang) {
    return instances.getOrDefault(
        lang.toLowerCase(), instances.get(RPGItems.plugin.cfg.language.toLowerCase()));
  }

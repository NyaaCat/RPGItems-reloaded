...
public class WGSupport {
        ...
        try {
            ...
            if (!wgVersion.startsWith("7.")) {
                RPGItems.logger.warning(
                        "Requires WorldGuard 7.0.0-beta2 or later, disabling integration");
                hasSupport = false;
                return;
            }
    ...
    public static class EventListener implements Listener {
        ...
        public <
                        ...
                        TReturn>
                void onPreTrigger(
                        RPGItemsPowersPreFireEvent<TEvent, TPower, TPimpl, TResult, TReturn>
                                event) {
            if (canUse(
                            event.getPlayer(),
                            event.getItem(),
                            event.getPowers(),
                            plugin.cfg.wgShowWarning)
                    == Event.Result.DENY) {

...
public class ConditionCommands extends RPGCommandReceiver {
  ...
  private static Pair<NamespacedKey, Class<? extends Condition<?>>> getConditionClass(
      CommandSender sender, String conditionStr) {
    try {
  ...
  public List<String> addCompleter(CommandSender sender, Arguments arguments) {
    ...
    switch (arguments.remains()) {
      ...
      case 2:
        completeStr.addAll(
            PowerManager.getConditions().keySet().stream()
      ...
      default:
        ...
        Pair<NamespacedKey, Class<? extends Condition<?>>> keyClass =
            getConditionClass(sender, conditionKey);
        if (keyClass != null) {
          return resolveProperties(
              sender, item, keyClass.getValue(), keyClass.getKey(), last, arguments, true);
        }
    }
  ...
  public void add(CommandSender sender, Arguments args) {
    ...
    Pair<NamespacedKey, Class<? extends Condition<?>>> keyClass =
        getConditionClass(sender, conditionStr);
    ...
    try {
      ...
      plugin
          ...
          .log(
              Level.WARNING,
              "Error adding condition " + conditionStr + " to item " + itemStr + " " + item,
              e);
  ...
  public List<String> propCompleter(CommandSender sender, Arguments arguments) {
    ...
    switch (arguments.remains()) {
      ...
      case 2:
        RPGItem item = getItem(arguments.nextString(), sender);
        completeStr.addAll(
            IntStream.range(0, item.getConditions().size())
                .mapToObj(i -> i + "-" + item.getConditions().get(i).getNamespacedKey())
                .collect(Collectors.toList()));
        break;
      default:
        ...
        return resolveProperties(
            sender,
            item,
            nextCondition.getClass(),
            nextCondition.getNamespacedKey(),
    ...
    if (next.contains("-")) {
      ...
      try {
        ...
        try {
          ...
          Condition<?> condition =
              item.getCondition(keyClass.getKey(), keyClass.getValue()).get(nth);
          if (condition == null) {
            throw new BadCommandException("message.condition.unknown", nth);
          }
          return condition;
  ...
  public static void showCondition(CommandSender sender, RPGItem item, Condition condition) {
    msgs(
        ...
        condition.id(),
        condition.getLocalizedName(sender),
        condition.getNamespacedKey().toString(),
        condition.displayText() == null
            ? I18n.getInstance(sender).format("message.power.no_display")
            : condition.displayText());
    PowerManager.getProperties(item.getPropertyHolderKey(condition))
        .forEach(
            (name, prop) ->
                showProp(sender, condition.getNamespacedKey(), prop.getValue(), condition));
  }
    ...
    stream.forEach(
        condition -> {
          ...
          msgs(
              ...
              PowerManager.getDescription(condition, null));
          PowerManager.getProperties(condition)
              .forEach((name, mp) -> showProp(sender, condition, mp.getValue(), null));

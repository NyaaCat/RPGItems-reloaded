...
public class ModifierCommands extends RPGCommandReceiver {
    ...
    private static Pair<NamespacedKey, Class<? extends Modifier>> getModifierClass(
            CommandSender sender, String modifierStr) {
        try {
    ...
    public List<String> addCompleter(CommandSender sender, Arguments arguments) {
        ...
        switch (arguments.remains()) {
            ...
            case 2:
                completeStr.addAll(
                        PowerManager.getModifiers().keySet().stream()
    ...
    public void add(CommandSender sender, Arguments args) {
        ...
        Pair<Pair<ItemStack, ItemMeta>, PersistentDataContainer> rootContainer =
                getRootContainer(sender, args, baseStr);
        PersistentDataContainer container = rootContainer.getValue();
        String modifierStr = args.nextString();

        Pair<NamespacedKey, Class<? extends Modifier>> keyClass =
                getModifierClass(sender, modifierStr);
        ...
        try {
            ...
            plugin.getLogger()
                    .log(
                            Level.WARNING,
                            "Error adding modifier " + modifierStr + " to " + baseStr + " ",
                            e);
    ...
    private NamespacedKey nextAvailable(PersistentDataContainer modifierContainer) {
        ...
        for (NamespacedKey key = PowerManager.parseKey(String.valueOf(i));
                modifierContainer.has(key, PersistentDataType.TAG_CONTAINER);
    ...
    private Pair<Pair<ItemStack, ItemMeta>, PersistentDataContainer> getRootContainer(
            CommandSender sender, Arguments arguments, String baseStr) {
            ...
            case 2:
                {
                    ...
                    Pair<Pair<ItemStack, ItemMeta>, PersistentDataContainer> rootContainer =
                            getRootContainer(sender, arguments, baseStr);
                    PersistentDataContainer container = rootContainer.getValue();
                    SubItemTagContainer modifierContainer =
                            ItemTagUtils.makeTag(container, TAG_MODIFIER);
                    List<Modifier> modifiers = RPGItem.getModifiers(modifierContainer);
                    completeStr.addAll(
                            modifiers.stream().map(Modifier::id).collect(Collectors.toList()));
                    break;
                }
            default:
                {
                    ...
                    Pair<Pair<ItemStack, ItemMeta>, PersistentDataContainer> rootContainer =
                            getRootContainer(sender, arguments, baseStr);
                    PersistentDataContainer container = rootContainer.getValue();
                    Pair<Integer, Modifier> nextModifier =
                            nextModifier(container, sender, arguments);
                    Modifier modifier = nextModifier.getValue();
                    return resolveProperties(
                            sender,
                            null,
                            modifier.getClass(),
                            modifier.getNamespacedKey(),
                            arguments.getRawArgs()[arguments.getRawArgs().length - 1],
                            arguments,
                            false);
                }
    ...
    public void prop(CommandSender sender, Arguments args) throws IllegalAccessException {
        ...
        try {
            ...
            NamespacedKey namespacedKey =
                    PowerManager.parseKey(String.valueOf(modifierPair.getKey()));
    ...
    public void showModifier(CommandSender sender, Modifier modifier) {
        msgs(
                sender,
                "message.modifier.show",
                modifier.getLocalizedName(sender),
                modifier.getNamespacedKey().toString(),
                modifier.id());
        NamespacedKey modifierKey = modifier.getNamespacedKey();
        PowerManager.getProperties(modifierKey)
                .forEach((name, prop) -> showProp(sender, modifierKey, prop.getValue(), modifier));
    }

    public Pair<Integer, Modifier> nextModifier(
            PersistentDataContainer container, CommandSender sender, Arguments args) {
        SubItemTagContainer modifierContainer = ItemTagUtils.makeTag(container, TAG_MODIFIER);
        List<Modifier> modifiers = RPGItem.getModifiers(modifierContainer);
        String next = args.nextString();
        OptionalInt index =
                IntStream.range(0, modifiers.size())
                        .filter(i -> modifiers.get(i).id().equals((next)))
                        .findFirst();
        if (!index.isPresent()) {
        ...
        stream.forEach(
                modifier -> {
                    ...
                    msgs(
                            ...
                            PowerManager.getDescription(modifier, null));
                    PowerManager.getProperties(modifier)
                            .forEach((name, mp) -> showProp(sender, modifier, mp.getValue(), null));

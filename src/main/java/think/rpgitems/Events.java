...
public class Events implements Listener {
  ...
  public void onHangingBreak(HangingBreakEvent e) {
    if (e.getCause().equals(RemoveCause.EXPLOSION))
      if (e.getEntity().hasMetadata("RPGItems.Rumble")) {
        e.getEntity()
            .removeMetadata("RPGItems.Rumble", plugin); // Allow the entity to be broken again
        e.setCancelled(true);
      }
  }
  ...
  @EventHandler
  public void onBreak(
      BlockPhysicsEvent e) { // Is not triggered when the block a torch is attached to is removed
  ...
  public void onPlayerShootBow(EntityShootBowEvent e) {
    ...
    if (e.getForce() != 0) {
      e.getProjectile()
          .setMetadata("RPGItems.OriginalForce", new FixedMetadataValue(plugin, e.getForce()));
    }
  ...
  public void onProjectileFire(ProjectileLaunchEvent e) {
    ...
    if (!(shooter instanceof Player)
        && !((shooter instanceof Entity)
            && projectileRegisterMap.containsKey(((Entity) shooter).getUniqueId()))) return;
    OfflinePlayer ofp =
  ...
  private void playerDamager(EntityDamageByEntityEvent e) {
    ...
    if (sourceItem.isPresent()) {
      ...
      sourceItem =
          Optional.ofNullable(
              (ItemStack) Context.instance().get(player.getUniqueId(), DAMAGE_SOURCE_ITEM));
      if (sourceItem.isPresent()) {
        item = sourceItem.get();
      }
    }
    ...
    if (suppressMelee.isPresent() && suppressMelee.get()) {
      ...
      suppressMelee =
          Optional.ofNullable(Context.instance().getBoolean(player.getUniqueId(), SUPPRESS_MELEE));
      if (suppressMelee.isPresent() && suppressMelee.get()) {
        overridingDamage.ifPresent(e::setDamage);
        return;
      }

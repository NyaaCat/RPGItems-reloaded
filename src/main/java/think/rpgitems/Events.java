...
public class Events implements Listener {
    ...
    public static void registerRPGProjectile(
            RPGItem rpgItem, ItemStack itemStack, Player player, LivingEntity source) {
    ...
    public void onHangingBreak(HangingBreakEvent e) {
        if (e.getCause().equals(RemoveCause.EXPLOSION))
            if (e.getEntity().hasMetadata("RPGItems.Rumble")) {
                e.getEntity()
                        .removeMetadata(
                                "RPGItems.Rumble", plugin); // Allow the entity to be broken again
                e.setCancelled(true);
            }
    }
    ...
    public void onBreak(
            BlockPhysicsEvent
                    e) { // Is not triggered when the block a torch is attached to is removed
    ...
    public void onBlockBreak(BlockBreakEvent e) {
        ...
        if (block.getType().equals(Material.TORCH))
            if (block.hasMetadata("RPGItems.Torch")) e.setCancelled(true);
                    ...
                    .runTask(
                            ...
                            () -> {
                                if (e.getHitEntity() != null
                                        ...
                                        && ((AbstractArrow) e.getEntity()).getPierceLevel() > 0) {
                                    return;
                                }
        ...
        if (rpgProjectiles.containsKey(entity.getEntityId())) {
            try {
                if (entity instanceof Trident
                        && entity.getScoreboardTags().contains("rgi_projectile")) {
                    ((Trident) entity).setPickupStatus(AbstractArrow.PickupStatus.DISALLOWED);
                }
                RPGItem rItem =
                        ItemManager.getItem(rpgProjectiles.get(entity.getEntityId())).orElse(null);

                if (rItem == null || !(entity.getShooter() instanceof Player)) return;
                ...
                Bukkit.getScheduler()
                        .runTask(
                                ...
                                () -> {
                                    if (e.getHitEntity() != null
                                            && e.getEntity() instanceof AbstractArrow
                                            && ((AbstractArrow) e.getEntity()).getPierceLevel()
                                                    > 0) {
                                        return;
                                    }
    ...
    public void onPlayerShootBow(EntityShootBowEvent e) {
        ...
        if (entity instanceof Player) {
            ...
            force =
                    rpgItem.flatMap(
                                    rpgItem1 -> {
                                        registerRPGProjectile(
                                                e.getProjectile().getEntityId(), rpgItem1.getUid());
                                        return rpgItem1.power(
                                                ((Player) entity), bow, e, BaseTriggers.BOW_SHOOT);
                                    })
                            .orElse(force);
            if (e.isCancelled()) {
        ...
        if (e.getForce() != 0) {
            e.getProjectile()
                    .setMetadata(
                            "RPGItems.OriginalForce", new FixedMetadataValue(plugin, e.getForce()));
        }
    ...
    public void onProjectileFire(ProjectileLaunchEvent e) {
        ...
        if (!(shooter instanceof Player)
                && !((shooter instanceof Entity)
                        && projectileRegisterMap.containsKey(((Entity) shooter).getUniqueId())))
            return;
        OfflinePlayer ofp =
                (shooter instanceof Player)
                        ? (Player) shooter
                        : Bukkit.getOfflinePlayer(
                                projectileRegisterMap.get(((LivingEntity) shooter).getUniqueId()));
        if (!ofp.isOnline()) {
    ...
    public void onPlayerAction(PlayerInteractEvent e) {
        ...
        if ((action == Action.RIGHT_CLICK_AIR || action == Action.RIGHT_CLICK_BLOCK)
                && (im == Material.BOW
                        || im == Material.SNOWBALL
                        || im == Material.EGG
                        || im == Material.POTION
                        || im == Material.TRIDENT)) return;
        RPGItem rItem = ItemManager.toRPGItem(e.getItem()).orElse(null);
        if (rItem == null) return;
        Entity playerTarget = RayTraceUtils.getTargetEntity(player);
        if (!(playerTarget instanceof ItemFrame)
                && (im.isEdible()
                        || im.isRecord()
                        || isPlacable(im)
                        || isItemConsumer(e.getClickedBlock()))) {
            e.setCancelled(true);
        }
    ...
    public void onPlayerSneak(PlayerToggleSneakEvent e) {
        ...
        Stream.of(armorContents).forEach(i -> trigger(p, e, i, trigger));
    }
    ...
    public void onPlayerSprint(PlayerToggleSprintEvent e) {
        ...
        ItemStack[] armorContents = p.getInventory().getArmorContents();
        Stream.of(armorContents).forEach(i -> trigger(p, e, i, sprint));
    }
    ...
    public void onPlayerPickupTrident(PlayerPickupArrowEvent e) {
        ...
        if (!rpgProjectiles.containsKey(e.getArrow().getEntityId())
                || !itemMeta.hasLore()
                || itemMeta.getLore().isEmpty()) {
            return;
        }
        try {
            ...
            if (realItem != null) {
                if (realItem.getType() == Material.AIR) {
                    ...
                    e.setCancelled(true);
                    Bukkit.getScheduler()
                            .runTaskLater(RPGItems.plugin, () -> e.getArrow().remove(), 100L);
                } else {
            ...
            logger.log(
                    Level.WARNING, "Exception when PlayerPickupArrowEvent. May be harmless.", ex);
        }
    }
    ...
    public void checkEnchantPerm(Cancellable e, HumanEntity p, RPGItem itemStack) {
        ...
        if (enchantMode == RPGItem.EnchantMode.DISALLOW) {
            e.setCancelled(true);
        } else if (enchantMode == RPGItem.EnchantMode.PERMISSION
                && !(p.hasPermission("rpgitem.enchant." + itemStack.getName())
                        || p.hasPermission("rpgitem.enchant." + itemStack.getUid()))) {
            e.setCancelled(true);
        }
    }
    ...
    public void onInventoryOpen(final InventoryOpenEvent e) {
        if (e.getInventory().getHolder() == null || e.getInventory().getLocation() == null) return;
    ...
    private void playerDamager(EntityDamageByEntityEvent e) {
        ...
        Optional<Boolean> suppressMelee =
                LightContext.getTemp(player.getUniqueId(), SUPPRESS_MELEE);
        Optional<Double> overridingDamage =
                LightContext.getTemp(player.getUniqueId(), OVERRIDING_DAMAGE);
        Optional<ItemStack> sourceItem =
                LightContext.getTemp(player.getUniqueId(), DAMAGE_SOURCE_ITEM);

        if (!overridingDamage.isPresent()) {
            overridingDamage =
                    Optional.ofNullable(
                            Context.instance().getDouble(player.getUniqueId(), OVERRIDING_DAMAGE));
        }
        ...
        if (sourceItem.isPresent()) {
            ...
            sourceItem =
                    Optional.ofNullable(
                            (ItemStack)
                                    Context.instance()
                                            .get(player.getUniqueId(), DAMAGE_SOURCE_ITEM));
            if (sourceItem.isPresent()) {
                item = sourceItem.get();
            }
        }
        ...
        if (suppressMelee.isPresent() && suppressMelee.get()) {
            ...
            suppressMelee =
                    Optional.ofNullable(
                            Context.instance().getBoolean(player.getUniqueId(), SUPPRESS_MELEE));
            if (suppressMelee.isPresent() && suppressMelee.get()) {
                overridingDamage.ifPresent(e::setDamage);
                return;
            }
        ...
        if (rItem != null && e.getCause().equals(EntityDamageEvent.DamageCause.BLOCK_EXPLOSION)
            ...
            double dmg =
                    damageMin < damageMax
                            ? ThreadLocalRandom.current().nextDouble(damageMin, damageMax)
                            : damageMax;
        ...
        runGlobalHitTrigger(
                e, player, damage, rItem == null ? "" : rItem.getDamageType(), inventory);
    }
    ...
    private void projectileDamager(EntityDamageByEntityEvent e) {
        ...
        if (projectileID == null) {
            if (projectile.hasMetadata("RPGItems.OriginalForce")) {
                double damage =
                        e.getDamage()
                                * projectile.getMetadata("RPGItems.Force").get(0).asFloat()
                                / projectile.getMetadata("RPGItems.OriginalForce").get(0).asFloat();
                e.setDamage(damage);
            }
            return;
        }
        RPGItem rItem = ItemManager.getItem(projectileID).orElse(null);
        if (rItem == null || !(projectile.getShooter() instanceof Player)) return;
        ...
        Boolean suppressProjectile =
                Context.instance().getBoolean(player.getUniqueId(), SUPPRESS_PROJECTILE);
    ...
    private void runGlobalHitTrigger(

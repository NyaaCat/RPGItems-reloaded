...
public class NetworkUtils {
  ...
  public static Map<String, String> downloadGist(String id, String token)
    ...
    CompletableFuture<FullHttpResponse> response =
        HttpClient.get("https://api.github.com/gists/" + id, headers);
    ...
    try {
      if (httpResponse.status().code() == 404) {
        ...
        throw new AdminCommands.CommandException(
            "message.import.gist.code", httpResponse.status().code());
      }
  ...
  public static Map<String, String> downloadUrl(String url)
      throws InterruptedException, ExecutionException, TimeoutException {
    CompletableFuture<FullHttpResponse> response = HttpClient.get(url, null);
    FullHttpResponse httpResponse = response.get(10, TimeUnit.SECONDS);
    try {
      if (httpResponse.status().code() == 404) {
        throw new AdminCommands.CommandException("message.import.url.notfound", url);
      } else if (httpResponse.status().code() != 200) {
        throw new AdminCommands.CommandException(
            "message.import.url.code", httpResponse.status().code());
      }
  ...
  public static String publishGist(
      Map<String, Map<String, String>> files, String token, String description, boolean isPublic)
      throws InterruptedException, ExecutionException, TimeoutException {
    Map<String, String> headers = new HashMap<>();
    headers.put("Authorization", "token " + token);
    ...
    CompletableFuture<FullHttpResponse> response =
        HttpClient.postJson("https://api.github.com/gists", headers, json);
    FullHttpResponse httpResponse = response.get(10, TimeUnit.SECONDS);
    try {
      if (httpResponse.status().code() != 201) {
        RPGItems.plugin
            .getLogger()
            .warning("Code " + httpResponse.status().code() + " publishing gist");
        RPGItems.plugin.getLogger().warning(httpResponse.content().toString(UTF_8));
        throw new AdminCommands.CommandException(
            "message.export.gist.code", httpResponse.status().code());
      }
  ...
  @SuppressWarnings("unchecked")
  public static Pair<String, List<String>> updateCommand(String item, String command)
      throws InterruptedException, ExecutionException, TimeoutException {
    ...
    CompletableFuture<FullHttpResponse> response =
        HttpClient.request(
            endPoint + "/command/112/113",
            HttpMethod.POST,
            null,
            Unpooled.wrappedBuffer(command.getBytes(UTF_8)),
    ...
    try {
      ...
      if (httpResponse.status().code() != 200) {
        ...
        throw new AdminCommands.CommandException(
            "message.spu.command.code", item, httpResponse.status().code(), body, command);
      }
  ...
  public static String updateEntity(String item, String entity, boolean name)
      throws InterruptedException, ExecutionException, TimeoutException {
    String endPoint = RPGItems.plugin.cfg.spuEndpoint;
    if (Strings.isNullOrEmpty(endPoint)) {
      throw new AdminCommands.CommandException("message.spu.no_endpoint");
    ...
    CompletableFuture<FullHttpResponse> response =
        HttpClient.request(
            endPoint + (name ? "/entityname/112/113" : "/entitynbt/112/113"),
            HttpMethod.POST,
            null,
            Unpooled.wrappedBuffer(entity.getBytes(UTF_8)),
            HttpHeaderValues.TEXT_PLAIN);
    FullHttpResponse httpResponse = response.get(10, TimeUnit.SECONDS);
    try {
      ...
      if (httpResponse.status().code() != 200) {
        ...
        throw new AdminCommands.CommandException(
            "message.spu.entity.code", item, httpResponse.status().code(), result, entity);
      }

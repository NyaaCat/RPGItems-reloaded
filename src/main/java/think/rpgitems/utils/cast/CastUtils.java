...
public class CastUtils {
   ...
   */
  @Deprecated
  public static CastLocation rayTrace(
      LivingEntity from, Location fromLocation, Vector towards, double range) {
    return rayTrace(fromLocation, towards, range, from);
  }

  public static CastLocation rayTrace(
      Location fromLocation, Vector towards, double range, LivingEntity from) {
    Vector hitPosition;
  ...
  public static Location parseFiringLocation(
      Location targetingLocation, Vector normalDir, Location direction, RoundedConeInfo coneInfo) {
  ...
  public static Vector makeCone(Location fromLocation, Vector towards, RoundedConeInfo coneInfo) {
    ...
    rotated.rotateAroundAxis(
        clone, Math.toRadians(coneInfo.getPhi() + coneInfo.getInitalRotation()));
    return rotated;
  }

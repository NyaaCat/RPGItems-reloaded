...
public class Handler implements CommandHandler {
    ...
    @CommandString("rpgitemupdate")
    @CommandDocumentation("$command.rpgitem.update")
    @CommandGroup("rpgitem_update")
    public void updateItemInHand(CommandSender sender) {
        ...
        if (!p.hasPermission("rpgitemupdate.command")) {
    ...
    @CommandString("rpgitemupdate inspect")
    @CommandDocumentation("$command.rpgitem.update")
    @CommandGroup("rpgitem_update")
    public void inspectItemInHand(CommandSender sender) {
        ...
        RPGItem rItem = _toRPGItem_Old(item);
        if (rItem == null) {
            p.sendMessage("Not a valid legacy RPGITEM");
        } else {
            p.sendMessage("Valid legacy RPGITEM: " + rItem.getName());
        }
    }

    @CommandString("rpgitemupdate upgrade")
    @CommandDocumentation("$command.rpgitem.update")
    @CommandGroup("rpgitem_update")
    ...
    @CommandString("rpgitemupdate downgrade")
    @CommandDocumentation("$command.rpgitem.update")
    @CommandGroup("rpgitem_update")
    ...
    private RPGItem _toRPGItem_Old(ItemStack item) {
        if (item == null)
            return null;
        if (!item.hasItemMeta())
            return null;
